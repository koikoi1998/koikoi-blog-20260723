...
```mermaid
graph LR
    Utility["電力会社<br/>(高圧AC)"] --> UPS["UPS<br/>(無停電電源装置)<br/>フロア/DC単位の大型設備、または<br/>ラック単位の小型機器のいずれか"]
    UPS --> PDU["PDU<br/>(配電盤)<br/>フロア単位の大型配電盤、または<br/>ラック単位の電源タップ相当(rPDU)のいずれか"]
...
なお、この図は電力が届くまでの経路を1本の線で示した簡略図です。実際には、後述する**A/Bグリッド冗長**を実現するため、UPSからPDUまでの経路をまるごと2系統（グリッドA・グリッドB）に分けて並走させ、1台のサーバーが両方から電力を受け取れるようにするのが一般的です。また、図中のUPS・PDUはいずれも、フロア（データセンター）単位で稼働する大型の設備として存在する場合と、ラック単位で完結する小型の機器として存在する場合の両方を指す言葉として使われており、両者は同じ経路上に必ず両方登場するものではなく、規模に応じてどちらかが選ばれる構成です。この粒度の違いと、UPS・PDUそれぞれの役割は「UPSとPDUの役割を深掘り」で改めて扱います。
...
<details>
<summary>電圧は「電子を押し出す力」なのか「電子が持つエネルギー」なのか（電位という考え方）</summary>

電圧の説明として「電子を押し出す力」（水圧のたとえ）と「電子1粒あたりが持つエネルギー」という2つの言い方が出てきましたが、これは矛盾する2つの定義ではなく、**「電位」という1つの量を、原因の側から見るか結果の側から見るかの違い**にすぎません。

ポイントは、**電圧（正確には電位）は特定の電子に固定でくっついている性質ではなく、回路上の「その場所」に紐づく性質**だということです。水道で言えば、これは「特定の水の分子が生まれつき持っているエネルギー」ではなく、「ダムの、その高さにいる」という位置エネルギーに相当します。高い位置にある水は、その位置にいるという理由だけで大きな位置エネルギーを持ち、同時に重力によって低い方へ押し出される力を受けています。「押し出す力」と「その水が持つエネルギー」は、実は同じ「高さ（＝位置エネルギー）」という1つの量の、2つの現れ方にすぎません。

電気回路も同じで、電池のプラス端子側は電位が高く、マイナス端子側は電位が低い状態にあります（この電位差が「電圧」です）。この電位差があるからこそ、その2点間を導線でつなぐと、電位の高い方から低い方へ電子を押しやる力（電場）が生まれます。そして、電子1粒がその電位差を移動しきったときに失う（または得る）エネルギーの量が、まさに「電圧×電荷1粒分」で計算されるエネルギーです。つまり「電圧＝押し出す力の大きさ」と「電圧＝電子が移動の過程で受け渡すエネルギーの大きさ」は、同じ電位差という現象を、力として見るかエネルギーとして見るかの違いであり、対立する説明ではありません。

**電池とは何をしている装置か**：電池は「決まった電圧を持つ特別な電子を送り出す装置」ではありません。電池の中で起きている化学反応が、まるでダムの下から上へ水をくみ上げるポンプのように、マイナス端子側にある電子を化学エネルギーを使ってプラス端子側へ絶えず汲み上げ続けており、その結果として両端子間に一定の電位差（＝電圧）を維持し続けている装置です。回路がつながって電子が電池の外を回り、マイナス端子に戻ってきても、電池はまたそれを化学反応でプラス端子側へ汲み上げ直すため、電位差が保たれたまま電流が流れ続けます。「摩擦がなければエネルギーが保存される」というイメージが浮かぶかもしれませんが、これはこの理解の延長線上で正しく、実際に何もない導線（抵抗ゼロ）を電子が移動するだけなら電位（＝位置エネルギー）は失われません。電位が下がる（＝エネルギーが失われる）のは、抵抗のような「電気的な摩擦」を電子が通過するときだけで、これが後述する「電圧降下」の正体です。

**「電圧をかける」とはどういうことか**：以上を踏まえると、「電圧をかける」とは、回路上のある2点の間に意図的に電位差を作り出す操作を指します。典型的には、電池や電源装置のプラス・マイナス端子をその2点に接続することがこれにあたります。逆に言えば、2点間に電位差が存在してさえいれば「電圧がかかっている」状態であり、その2点を導体でつなげば電位差を解消する向きに電流が流れ始めます（何もつながっていなければ、電位差は存在するのに電流は流れない、という状態もありえます）。

なお、この「電位差」という考え方はコンデンサにもそのまま当てはまります。コンデンサへの電圧のかけ方と、電極に電荷が溜まっていく仕組みそのものは、後述の「そもそも『コンデンサ』とは何をする部品なのか」でまとめて扱います。
...
電池のある回路の中に、この2枚の電極を置いた状態を図にすると、次のようになります。

```mermaid
graph LR
    Bat["電池"] -->|"+端子から導線"| PA["電極A<br/>(電子が引き抜かれプラスに帯電)"]
    Bat -->|"-端子から導線"| PB["電極B<br/>(電子が押し込まれマイナスに帯電)"]
    PA -.絶縁体(誘電体)で隔てられているため<br/>電荷はここを飛び越えて流れることはできない.- PB
```

電池のプラス端子につながった電極Aからは電子が引き抜かれてプラスに帯電し、マイナス端子につながった電極Bには電子が押し込まれてマイナスに帯電していきます。「片側にマイナスが集まることで、新たなマイナス（電子）が進めなくなる（S極とS極が反発するようなイメージ）」という理解は、直感としてほぼ正しい捉え方です。同じ符号の電荷（電子）が1箇所に集まるほど、そこへ新たな電子を押し込もうとする力への反発（電場による反発力）が強くなり、電極間の電位差が外部電圧に近づくにつれて、それ以上電子を送り込む「押し出す力」が相殺されていきます。これが、コンデンサが電圧をかけ続けても永久に充電され続けるのではなく、外部電圧と釣り合ったところで電流がほぼゼロに収束する理由です。逆に外部からの電圧が下がる・なくなる（コンデンサより電位の低い部分に導線でつながる）と、蓄えられていた電位差を解消しようとして、今度は蓄えていた電子が放出され、電流が流れます。これが「電圧が上がる／下がる」タイミング、すなわち「充電中は電圧が上昇し、放電中は電圧が低下する」という挙動の正体です。
...
**「損失＝電流²×抵抗」と「電力＝電圧×電流」は矛盾しないのか**

「損失＝電流²×抵抗」は`損失 = (電流×抵抗) × 電流`とも書けるので、「損失＝電圧×電流」に等しいはずであり、だとすれば電圧を上げても電流を上げても損失への影響は同じでは、という疑問が自然に浮かびます。ここでの注意点は、**2つの式に出てくる「電圧」は、実は別のものを指している**ということです。

- 送電の効率化のために意図的に上げ下げしているのは、**送電線に送り出す側の電圧**（発電所や変圧器が作り出す、送電区間全体にかかる電圧）であり、これが電力の式`電力 = 電圧 × 電流`の電圧です。
- 一方、`損失 = 電流 × 抵抗 × 電流`と書き換えたときの「電流×抵抗」の部分は、**送電線という抵抗体そのものに生じる電圧降下**であり、送電線の両端の電圧差にすぎません。これは電流と抵抗だけで受動的に決まる値であって、送電電圧のように人為的に設定できる変数ではありません。

つまり「電力を一定に保ったまま送電電圧を上げれば、電流はその分下がる（`電流 = 電力 ÷ 電圧`）」という関係は使えますが、「損失を減らすために電圧降下の方を直接操作する」という手段は存在しません。損失を減らす実質的なレバーは電流を下げることだけであり、電流を下げるために送電電圧を上げる、という一方向の因果関係になっています。式の上で同じ「電圧」という文字が使われていても、片方は人為的に操作する独立変数、もう片方は電流の結果として決まる従属変数という、性質の異なる量である点が、この疑問のポイントです。

**なぜ「電圧を上げる」という対策に、交流の変圧技術が関係してくるのか**

発電所や需要家（家庭・工場）で実際に必要とされる電圧は、安全性や機器の絶縁設計の都合上、送電に最適な超高電圧（数万〜数十万V）ではありません。そのため、送電損失を減らすには「送電区間だけ電圧を大きく引き上げ、需要家の手前で再び使用に適した電圧まで下げる」という、昇圧→送電→降圧という一連の運用が必須になります。この昇圧・降圧を、大容量かつ低損失・低コストで実現できる装置が変圧器（トランス）であり、変圧器は原理上、交流でなければ機能しません（理由は次項で説明します）。つまり「電圧を上げれば損失を減らせる」という理屈自体は電圧の大小の話でしかありませんが、それを実際の送電網で大容量に実行できるかどうかは「電圧を自在に上げ下げできる手段があるかどうか」にかかっており、その手段を提供しているのが交流の変圧器だ、という関係になっています。
...
なぜ「巻き数の比率を変えるだけ」で電圧を自在に変換できるのかというと、一次コイル・二次コイルのどちらも、**「1巻きあたりに誘導される電圧」は、同じ変化する磁束を共有しているために常に等しい**という性質があるためです。コイル全体の電圧は「1巻きあたりの電圧 × 巻き数」で決まるので、二次側の巻き数を一次側の2倍にすれば、二次側の電圧も自動的に2倍になります（このときエネルギー保存則により、二次側の電流は逆に1/2になり、電力`電圧×電流`自体は一次側・二次側で変わりません）。つまり「巻き数を変えるだけで電圧を変換できる」というのは、この「1巻きあたりの誘導電圧が一次・二次で共通である」という電磁誘導の性質があってこそ成立する話です。

DCは電流が常に一定方向・一定の大きさのままなので、この電磁誘導による電圧変換の仕組みがそのままでは使えず、DCの電圧を変えるには電子回路（DC-DCコンバータのような能動的なスイッチング回路）が別途必要になります。歴史的に長距離送電がACを基本に発展してきたのは、変圧器という単純で信頼性の高い機械的な装置だけで電圧変換ができたことが大きな理由です（現代では大陸間の海底送電などでHVDC:高圧直流送電も一部使われていますが、これは変換設備のコストを送電距離の長さで正当化できる特殊なケースです）。
...
前述のA/Bグリッド冗長を実現する際は、1本のPDUだけに全PSUを接続しては意味がありません。**グリッドA用のPDU（PDU-A）とグリッドB用のPDU（PDU-B）を物理的に2本に分け、それぞれ別系統のUPS・別系統のブレーカーに接続したうえで、1台のサーバーの複数のPSUを別々のPDUに挿し分けて初めて、配電経路の冗長化が成立します**。つまり「1本のPDUが電力をA系・B系に振り分けている」のではなく、「もとから物理的に独立したPDU-AとPDU-Bが並走しており、サーバー側が両方から1本ずつ電源ケーブルを引く」という構成である点に注意してください。

<details>
<summary>「UPS」「PDU」はそれぞれフロア単位・ラック単位の2つの粒度を指す（ラックにUPSはあるのにPDUが見当たらないのはなぜか）</summary>

「UPS」「PDU」という言葉は、どちらも現場では次のように**フロア（データセンター設備）単位の大型機器**と**ラック単位の小型機器**という、2つの異なる粒度を指して使われており、これが混同の元になりがちです。

- **フロアUPS（設備側の集中型UPS）**: サーバールーム・データセンターのフロアや、複数ラックからなる「UPSゾーン」単位でまとめて電力をバックアップする、大容量の据え置き型UPSです。`N+1`や`2N`といった冗長構成で運用されるのが一般的で、ラックの中には収まらない、建物設備の一部です。
- **ラックマウント型UPS**: ラックの中にサーバーと同居する形で設置される、そのラック1本分をカバーする小型のUPSです。
- **フロアPDU（設備側の配電盤）**: サーバールーム・データセンターのフロア単位で、UPSからの出力をいくつもの回路（ブレーカー）に分岐させる、壁や床下に設置された大型の配電盤です。トランスを内蔵し電圧変換まで行うタイプもあります。ラックの中には収まらない、建物設備の一部です。
- **ラックPDU（rPDU）**: ラックの中に取り付けられる、いわゆる「電源タップの業務用版」です。本記事の表で説明しているのはこちらの、ラック単位で見えるPDUです。

重要なのは、**この2つの粒度は同じ経路の中に必ず両方が積み重なって登場するわけではなく、データセンターの規模に応じてどちらか一方が選ばれる**という点です。大規模なデータセンターでは、フロア単位のUPS・PDUが電力会社からの電力をまとめて受け、そこからラックPDU（rPDU）経由で各サーバーに配電するのが一般的な構成です。一方、1つのラックにサーバーとUPSがラッキングされ、サーバー→UPS→床のコンセントという配線を採用している小規模な環境は、**フロアUPSの代わりにラックマウント型UPSを使い、かつラックPDU（rPDU）も省略した、最もシンプルな構成**にあたります。ラックマウント型UPS自体が背面に複数のコンセントを備えており、そのコンセント自体が事実上その1ラック分の電力分配点として機能しているため、ラック内に独立した「PDU」という箱を別途置く必要がなかったのだと考えられます。一方、そのラックマウント型UPSが電力を受け取っている「床のコンセント」の先には、目に見えない形でフロアPDU（またはそれに相当する分電盤）が必ず存在しており、そこから複数のラック・複数のUPSに向けて電力が分岐されています。つまり「ラックの中にPDUという装置が見当たらない」ことと、「電力会社→UPS→PDUという全体像が成立していない」ことは矛盾しません。PDUの役割を、目に見える場所ではなく1段階上流（壁の中・床下）が担っているだけです。

大規模データセンターでフロアUPS・ラックPDUの組み合わせが標準的に使われるのは、1つのラックに何台ものサーバーを搭載する密度の高い環境では、ラック単独でのバッテリーバックアップよりも設備全体を集中管理できるフロアUPSの方が保守性・拡張性に優れ、かつラックごとに電源タップ相当の分配点と、コンセント単位の消費電力計測・リモート電源サイクルといった機能が実務上必須になるためです。1ラックにサーバー数台程度の小規模環境では、ラックマウント型UPS+ラックPDU省略という、本記事で扱った構成も現実によく見られます。
...
**3. メインレールが出力されると、マザーボード上のVRMがそれをさらに細かい電圧（CPU用の1V前後、メモリ用の1.1V前後など）に変換します。** 電力供給が安定して初めてPOST（Power-On Self-Test）が実行され、OSの起動プロセスが始まります。POST完了後、ブートローダー・initramfs・systemdといった段階を経てOSが起動していく仕組みについては、別記事「[POST後のOS起動プロセスを『上位1%』の視点で理解する](/articles/os-boot-process-guide)」で深掘りしています。
