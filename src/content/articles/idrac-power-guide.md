...
なお、この図は電力が届くまでの経路を1本の線で示した簡略図です。実際には、後述する**A/Bグリッド冗長**を実現するため、UPSからPDUまでの経路をまるごと2系統（グリッドA・グリッドB）に分けて並走させ、1台のサーバーが両方から電力を受け取れるようにするのが一般的です。この点は「UPSとPDUの役割を深掘り」で改めて扱います。
...
**電池とは何をしている装置か**：電池は「決まった電圧を持つ特別な電子を送り出す装置」ではありません。電池の中で起きている化学反応が、まるでダムの下から上へ水をくみ上げるポンプのように、マイナス端子側にある電子を化学エネルギーを使ってプラス端子側へ絶えず汲み上げ続けており、その結果として両端子間に一定の電位差（＝電圧）を維持し続けている装置です。回路がつながって電子が電池の外を回り、マイナス端子に戻ってきても、電池はまたそれを化学反応でプラス端子側へ汲み上げ直すため、電位差が保たれたまま電流が流れ続けます。ご質問にあった「摩擦がなければエネルギーが保存される」というイメージはこの理解の延長線上で正しく、実際に何もない導線（抵抗ゼロ）を電子が移動するだけなら電位（＝位置エネルギー）は失われません。電位が下がる（＝エネルギーが失われる）のは、抵抗のような「電気的な摩擗」を電子が通過するときだけで、これが後述する「電圧降下」の正体です。
...
<details>
<summary>ラックにUPSはあるのにPDUが見当たらないのはなぜか（PDUという言葉が指す2つの粒度）</summary>

「PDU」という言葉は、現場では次の2つの異なる粒度を指して使われており、これが混同の元になりがちです。
...
ご質問の環境（1つのラックにサーバとUPSがラッキングされ、サーバー→UPS→床のコンセントという配線）は、この2つのうち**ラックPDU（rPDU）が省略された、小規模・シンプルな構成**にあたります。ラックマウント型UPS自体が背面に複数のコンセントを備えており、そのコンセント自体が事実上その1ラック分の電力分配点として機能しているため、ラック内に独立した「PDU」という箱を別途置く必要がなかったのだと考えられます。一方、UPSが電力を受け取っている「床のコンセント」の先には、目に見えない形でフロアPDU（またはそれに相当する分電盤）が必ず存在しており、そこから複数のラック・複数のUPSに向けて電力が分岐されています。つまり「ラックの中にPDUという装置が見当たらない」ことと、「電力会社→UPS→PDUという全体像が成立していない」ことは矛盾しません。PDUの役割を、目に見える場所ではなく1段階上流（壁の中・床下）が担っているだけです。

大規模データセンターでラックPDUが標準的に使われるのは、1つのラックに何台ものサーバーを搭載する密度の高い環境では、ラックごとに電源タップ相当の分配点と、コンセント単位の消費電力計測・リモート電源サイクルといった機能が実務上必須になるためです。1ラックにサーバー数台程度の小規模環境では、ラックPDUを省略してUPSのコンセントに直挿しする構成も現実によく見られます。
