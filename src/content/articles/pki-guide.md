...
description: "公開鍵暗号の非対称性、デジタル署名、CSRの作成、CAによる証明書発行、証明書チェーンの検証手順まで、TLSやVPNなど幅広い場面で使われるPKIの仕組みを体系的に理解する。"
series: "security"
order: 1
...
- **この記事で得られること**: 「証明書」という言葉の中身——公開鍵暗号がなぜ「鍵が2つ」で成立するのか、デジタル署名がどういう手順で「本人が作った・改ざんされていない」を証明するのか、CSR（証明書署名要求）に何が入っているのか、CA（認証局）が証明書を発行するとはどういう処理なのか、そして証明書チェーンの検証が具体的に何を確認しているのかを、体系的に理解できます。
