...
- **この記事で得られること**: [ネットワークスタックの仕組みを『上位1%』の視点で理解する](/articles/network-stack-guide) で「NICドライバはOSとハードウェアの橋渡し役」と説明した、その中身——パケットが届いたときにCPUが実際に何をしているのか（割り込み処理）、CPUを介さずデータを転送する仕組み（DMA）、NIC自身が代行してくれる処理（オフロード機能）、そしてOSのネットワークスタックそのものを経由しない超高速化技術（カーネルバイパス）まで、Linuxネットワーキングの最深部を体系的に理解できます。
- **対象読者**: 前提記事でNICドライバ・割り込み・DMAという言葉には触れたものの、「結局CPUは何をしていて、何をしていないのか」を説明できない方。この記事はネットワークスタックそのものではなく、**それを支えるLinuxカーネル側の実装**にフォーカスしています。
- **読むのにかかる想定時間**: 約25分
...
なお、「そのIPアドレスは、実際にはどのMACアドレスに対応するのか」を解決する仕組み（ARP）については、前提記事の[ネットワークスタックの仕組みを『上位1%』の視点で理解する — インターネット層：IPアドレスとARP](/articles/network-stack-guide)で扱っています。この記事では「なぜMACアドレスという仕組みそのものが必要なのか」に焦点を当て、「IPとMACをどう対応づけるか」は前提記事に譲ります。
...
大規模・高トラフィックな環境では、単に「割り込みを複数コアに分散する」だけでなく、**そのコアがNICに物理的に近いCPUソケット（NUMAノード）に属しているか**まで意識してチューニングします。NICがPCIeスロットを通じて特定のCPUソケットに物理的に近い位置に接続されている場合、そのソケットに属するコア・メモリで処理を完結させたほうが、ソケットをまたぐメモリアクセス（レイテンシが増える）を避けられます。`irqbalance`のようなツールは基本的な分散は行いますが、超高性能が求められる環境では手動でのアフィニティ設定・NUMA配置の検証が行われます。
