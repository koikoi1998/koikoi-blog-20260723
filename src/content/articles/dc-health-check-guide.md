...
なお、「エクスプローラーで`\\サーバー名\C$`へ管理者権限でアクセスすると、そのままADMIN$にもアクセスできるようになる」という理解は、半分正しく半分正確ではありません。**C$とADMIN$は独立した別々の共有であり、片方へのアクセスがもう片方の権限を"付与する"わけではありません。** ただし、SMB接続はサーバー単位で認証セッションを確立するため、一度管理者アカウントとして認証が成立すれば、**同じ認証セッションのまま、その管理者権限で許可されているC$・ADMIN$のどちらにも(再認証なしに)アクセスできる**ようになります。「C$がADMIN$を解錠する」のではなく、「同じ管理者セッションが、たまたまC$にもADMIN$にも通用する」という理解が正確です。

</details>

### NETLOGONとSYSVOL、そしてなぜ両方が必要なのか

ここで一度、`NETLOGON`という名前が指しうる2つの別のものを整理しておきます。ここまで説明してきた`NETLOGON`は**共有フォルダー**(SYSVOL配下の`scripts`フォルダーを公開したもの)でしたが、これとは別に**Netlogonサービス**というWindowsサービスが存在します。**Netlogonサービスは、そのDC自身のコンピューターアカウントによるセキュアチャネルの確立・維持、クライアントの認証要求の処理、そしてクライアントが自分の所属するDCを検索する際に使われるDNSのSRVレコードの動的登録、という3つの役割を担うサービス**です。`NETLOGON`共有の名前は、このNetlogonサービスがログオンスクリプトの配布処理にも関与していたという歴史的経緯に由来していますが、**共有そのものの実体はSYSVOLの一部であり、Netlogonサービスが共有フォルダーの中身を保持しているわけではありません**。セキュアチャネルの仕組みそのものは[Netlogonサービスとセキュアチャネルの仕組みを『上位1%』の視点で理解する](/articles/ad-netlogon-guide)で深掘りしています。
