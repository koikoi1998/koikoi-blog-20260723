...
- **この記事で得られること**: 「RESTful API」「HTTPS」「JSON」という、よく見かけるが説明を求められると詰まりがちな用語について、HTTPの基礎からREST設計の思想、認証・冪等性・バージョニングといった実務レベルの設計論まで体系的に理解できます。
- **対象読者**: インフラエンジニアとして就業しており、「APIはURLに対してGET/POSTするものだ」というレベルの理解はあるものの、なぜそのような設計になっているのか、何を守ればRESTfulと呼べるのかを説明できない方を想定しています。
- **読むのにかかる想定時間**: 約20分

この記事は[『上位1%』シリーズ 全記事ガイド](/sitemap)の一部です。

「APIはURLに対してGET/POSTするものだ」という理解の、その「同じ通信方式」の中身——つまりHTTP・REST・JSONが具体的に何をしているのかを、この記事で掘り下げていきます。
...
<details>
<summary>「プロトコル」と「API」は何が違うのか</summary>

この2つは階層が異なる別の概念であり、**APIはプロトコルの上に成り立っています**。

- **プロトコル**は、2つの機器・ソフトウェアが通信するときに守るべき「手順・形式のルールそのもの」を指します。「こういう順番で、こういう形式のリクエストとレスポンスをやり取りする」という、いわば通信の文法・作法です。HTTPやTCPはプロトコルの代表例で、特定のソフトウェアに依存せず、どんなクライアント・サーバーの組み合わせでも共通して使える汎用的なルールです。
- **API**は、あるソフトウェア（サーバー）が「自分に対してこういう操作ができますよ」と外部に公開している、個別の窓口・約束事を指します。「このURLにGETすればユーザー情報が取れる」「このURLにPOSTすればユーザーを作成できる」というのは、そのサービス固有の取り決めであり、他のサービスのAPIとは内容が異なります。

たとえるなら、プロトコルは「電話のかけ方・話し方の作法（相手が出たら名乗る、要件を話す、など）」、APIは「特定の会社に電話したときに聞ける個別の窓口（このダイヤルを押せば注文受付、あのダイヤルを押せば返品受付）」のようなものです。作法（プロトコル）は会社が違っても共通ですが、窓口の中身（API）は会社ごとに異なります。RESTful APIは、「HTTPというプロトコルが定める作法に則って、個々のサービスが独自のAPI（窓口）を公開する」という組み合わせで成立している、という理解が正確です。
...
公開鍵暗号方式そのものの仕組み(なぜ鍵が2つあるだけで安全性が成立するのか)や、証明書・CSR・証明書チェーンの検証手順まで踏み込んで理解したい場合は、[PKIとデジタル証明書の仕組みを『上位1%』の視点で理解する](/articles/pki-guide)で体系的に扱っています。
...
**統一インターフェース（HTTPメソッドの意味と冪等性）**

| メソッド | 意味 | 冪等性（同じリクエストを何度送っても結果が同じか） |
|---|---|---|
| GET | リソースを取得する | 冪等（何度実行しても状態は変わらない） |
| POST | リソースを新規作成する | 非冪等（実行するたびに新しいリソースが作られうる） |
| PUT | リソースを丸ごと置き換える | 冪等（同じ内容で何度送っても結果は同じ） |
| PATCH | リソースの一部だけを更新する | 実装依存（冪等になるよう設計するのが望ましい） |
| DELETE | リソースを削除する | 冪等（削除済みのものを再度削除しても状態は変わらない） |
...
**具体例で考えてみましょう。** カレンダー管理アプリ(クライアント)に「Googleカレンダーと連携する」というボタンがあったとします。これを押すと、カレンダー管理アプリのサーバーではなく、**Googleのログイン画面**(認可サーバー)にリダイレクトされます。ここであなたが入力するGoogleアカウントのパスワードは、Googleのサーバーだけが受け取り、カレンダー管理アプリには一切渡りません。ログイン後、「このアプリにGoogleカレンダーの予定の読み取り・書き込みを許可しますか?」という同意画面が表示され、「許可する」を押すと、カレンダー管理アプリは前述の認可コードを経てアクセストークンを受け取ります。以降、カレンダー管理アプリはこのアクセストークンを使ってGoogleカレンダーのAPI(リソースサーバー)にアクセスしますが、**Googleアカウントのパスワードそのものは、カレンダー管理アプリ側には一度も渡っていない**、という点が実務上の最大のメリットです。仮にカレンダー管理アプリのデータベースが漏洩しても、漏れるのは範囲の限定されたアクセストークンだけで、Googleアカウントのパスワードそのものが漏れることはありません。
...
<details>
<summary>「APIサーバー」は、サービスを提供するサーバーとは別に用意されるものなのか</summary>

ここまで「APIサーバー」という言葉を、特にそれ以上の説明なく使ってきましたが、実務では「サービスを提供する目的のサーバー(Webサイトを表示する、注文を処理するなど)」と「APIの窓口だけを担当するサーバー」を、意図的に分離することが一般的になっています。この分離を担うコンポーネントは、**APIゲートウェイ**と呼ばれます。

APIゲートウェイの役割は、次のようなものです。

- クライアントからのリクエストを受け付け、URLやヘッダーの内容に応じて、裏側にある適切なサービス(マイクロサービス)へ転送する
- 認証・認可を、個々のサービスではなくゲートウェイ側で一括して行う
- レートリミットの適用や、リクエストログの記録を一元化する
- 複数の裏側サービスへのリクエストを集約し、クライアントには1つのレスポンスとしてまとめて返す(BFF: Backend For Frontendと呼ばれるパターン)

**この役割は、[プロキシとファイアウォールの使い分けを『上位1%』の視点で理解する](/articles/proxy-firewall-guide)で扱ったリバースプロキシと、非常によく似ています。** 実際、APIゲートウェイの多くは、内部的にはリバースプロキシの機能を土台にして作られています。両者の違いは、リバースプロキシが「HTTPリクエストをどこへ転送するか」という汎用的な役割に徹しているのに対し、APIゲートウェイは、それに加えてAPIキーの検証、リクエスト/レスポンスの形式変換、複数サービスの集約といった、API特有の処理まで担う、より専門化されたリバースプロキシだという点です。「APIサーバーを分離するかどうか」は、システムの規模と、裏側のサービスが1つか複数かによって決まります。裏側が1つのサービスだけであれば、そのサービス自身がAPIを直接公開して問題ありませんが、複数のマイクロサービスへ分割された規模になると、APIゲートウェイによる一元化が実務上ほぼ必須になります。
...
ネットワークは信頼できるとは限らず、リクエストを送ったのにタイムアウトでレスポンスが受け取れない、ということが起こり得ます。このとき「同じリクエストをもう一度送ってよいか」を判断する基準が冪等性です。`PUT`や`DELETE`は冪等なので安全にリトライできますが、`POST`は非冪等なため、単純にリトライすると同じリソースが二重作成される危険があります。大規模なAPI連携やリトライ処理を設計する際は、この冪等性の区別が事故を防ぐ土台になります。
...
ここで「届いていないかもしれないから」と単純に`POST`をもう一度送ると、2のケースでは送金が2回実行されてしまいます。これが「`POST`は非冪等だから単純リトライは危険」の具体的な意味です。実務では、クライアント側でリクエストごとに一意な**冪等性キー**（Idempotency-Key、UUID等をリクエストヘッダーに含める）を発行し、サーバー側は同じキーのリクエストを受け取ったら「新規に処理するのではなく、前回と同じ結果を返す」ように実装することで、`POST`のような本来非冪等な操作にも安全にリトライできる仕組みを持たせます。これは多くの決済API（Stripe等）で標準的に採用されている設計です。決済APIがなぜPaymentIntentのような多段階の状態遷移を持つ設計になっているのか、そして決済完了をWebhookで検知する仕組みについては、[決済APIの裏側の仕組みを『上位1%』の視点で理解する](/articles/payment-api-guide)で深掘りしています。
...
- クライアント側でレートリミット・タイムアウト・リトライ（冪等なメソッドのみ）を適切に実装する。
...
- 冪等性・ページネーション・レートリミット・バージョニングは、APIを大規模に・安全に使いこなすための実務知識です。

**今日から意識すべきこと**
1. 普段触っているAPIのHTTPメソッドとステータスコードを、意識して確認してみましょう。
2. 自動化スクリプトを書くときは、そのAPI呼び出しが冪等かどうかを意識し、リトライ処理を安全に実装しましょう。
