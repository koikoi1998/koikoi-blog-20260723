...
- **IKE(Internet Key Exchange)** : 暗号鍵を安全に合意し、通信相手を認証するためのプロトコルです。UDPポート500(NAT越え時は4500)を使います。
...
ここからは、日本国内の本社にCisco機器、海外拠点にWatchGuard Fireboxを設置し、両拠点をIPsecの拠点間VPNで接続するという、実務でよくあるベンダー混在シナリオを具体的に見ていきます。異なるベンダーの機器同士を接続する際に最初にぶつかる壁は、**用語がベンダーごとにまったく異なる**ことです。同じ役割を指す設定項目でも、Ciscoの資料とWatchGuardの資料では呼び方が違うため、まずは対応関係を押さえる必要があります。
