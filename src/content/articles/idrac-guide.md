...
### WebサーバーとしてのiDRAC：なぜブラウザだけでGUI画面が見えるのか
...
<details>
<summary>仕組みをもう少し詳しく（iDRAC内部のWebサーバーの構造）</summary>
...
なお、この「デスクトップ環境がなくても、小さなWebサーバープロセスとファイル群、ネットワークソケットさえあればブラウザにGUIを配信できる」という発想は、iDRACに限らずProxmox VE・ルーター・NASなど、ブラウザで管理画面を持つ機器全般に共通する仕組みです。
...
### 仮想KVM・仮想コンソールとは何か（Linuxの「KVM」との違い）

「仮想KVM」「仮想コンソール」は、iDRACの文脈ではほぼ同じ意味で使われている表現で、実機にモニタ・キーボードを物理的に接続しなくても、ネットワーク越しにホストの画面操作ができる機能を指します。

<details>
<summary>「KVM」という言葉の紛らわしさと、技術的な仕組み</summary>

この「KVM」という略語には、混同しやすい2つの全く異なるルーツがあります。

**1. 元々のKVM（Keyboard, Video, Mouse）**

1990年代から存在する「KVMスイッチ」というハードウェア機器に由来する言葉で、1台のキーボード・モニタ・マウスを複数のサーバーで切り替えて共有するための装置を指します（Keyboard/Video/Mouseの頭文字）。データセンターのラックに、1台のモニタとキーボードだけを設置し、そこから物理的なスイッチでどのサーバーを操作するか切り替える、という運用は今も現場にあります。

**2. Linuxカーネルの仮想化基盤「KVM（Kernel-based Virtual Machine）」**

Proxmox VE（PVE）などが使っているハイパーバイザー技術で、Linuxカーネルに組み込まれた仮想マシン実行基盤を指します。こちらは「Kernel-based Virtual Machine」の頭文字です。

この2つは**綴りが同じだけの、由来も技術的背景も全く異なる別物**です（頭文字がたまたま一致しているだけの偶然です）。iDRACの「仮想KVM」「仮想コンソール」は前者（Keyboard/Video/Mouseスイッチ機能のネットワーク越し版）の系譜であり、後者（Linuxのハイパーバイザー技術）とは無関係です。iDRAC自体は組み込みLinuxで動作していますが、PVEのようにゲストVMを構築・実行するハイパーバイザー環境ではなく、iDRAC上で仮想マシンを動かせるわけではありません。
