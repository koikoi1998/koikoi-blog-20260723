...
管理者がiDRAC経由、あるいは物理の電源ボタンで「電源ON」を指示すると、その信号がPSU（電源ユニット）に伝わり、PSUはそれまでのスタンバイ電源（数W程度）に加えて、CPU・メモリ・PCIeカードなどを動かすためのメインの電源レール（+12V系統など）の出力を開始します。マザーボード上のVRM（電圧レギュレータモジュール）がその+12Vを各コンポーネントが必要とする電圧（CPU用の1V前後など）にさらに変換して供給し、電力が安定して初めてPOST（Power-On Self-Test）が走り、OSの起動プロセスが始まります。「電源ケーブルが刺さっていれば常に一定の電力が流れ込み続ける」わけではなく、PSUは一定の電圧を保ちながら、その時々でホストが要求する分だけの電流を流している、というのが正確な理解です。この電力変換の仕組み（AC/DC変換の必要性、PSUの冗長化設計）を図解つきでさらに詳しく知りたい方は、別記事「[サーバー電源の仕組みを『上位1%』の視点で理解する](/articles/idrac-power-guide)」で深掘りしています。また、POST完了後にブートローダー・initramfs・systemdを経てOSが起動していく仕組み自体は、別記事「[POST後のOS起動プロセスを『上位1%』の視点で理解する](/articles/os-boot-process-guide)」で扱っています。
...
**では、iSMをインストールしていない場合、この内部経路(USB NIC/KCS)はまったく使われないまま無駄になっているのでしょうか。** 答えは「経路の種類による」です。**KCS(Keyboard Controller Style)インターフェースは、そもそもiSM専用の仕組みではありません。** KCSはIPMI仕様そのものが定義する「システムインターフェース」の1つであり、iSMというDell独自のソフトウェアが登場するよりも前から、ベンダーを問わずBMC全般が備える標準的な経路です。Linuxの`ipmitool`やOpenIPMIドライバ、Windowsの標準IPMIドライバといった、iSMとはまったく関係のない汎用IPMIツール・OS標準機能もこのKCS経由でBMCと通信しており、さらにサーバーのBIOS/UEFI自体も、起動シーケンス中のPOST進捗通知やウォッチドッグタイマーのリセットなどに、この経路を日常的に使っています。つまり**KCS経路そのものは、iSMの有無にかかわらずOS・ファームウェアレベルで常に使われており、決して無駄な設計ではありません。**

一方、**USB NIC(内部USB接続の仮想的なネットワークインターフェース)は、KCSより高速なDell独自の拡張チャネル**で、iSM固有の機能(ローカルRACADMの橋渡し、Lifecycle LogのOSイベントログへの複製など)のために使われます。iSMをインストールしていない環境では、この**USB NIC経路自体は物理的に存在していても、通信の相手となるiSMプロセスがOS側に存在しないため、実質的に使われないまま待機している状態**になります。「もったいないのでは」という感覚は、この拡張チャネルに関しては的を射ており、iSM固有の付加価値機能を使わない環境では、この部分は活用されずに眠ったままになる、というのが正確な理解です。ただし土台となるKCS経路自体は前述の通り常に稼働しているため、「内部経路全体が無駄になっている」わけではなく、「iSM専用の拡張部分だけが未使用のまま残っている」というのがより正確な表現です。
