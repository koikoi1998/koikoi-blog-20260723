...
- **WANポート**: ルーター内部で「WAN側インターフェース」として扱われ、ISPからDHCPまたはPPPoEでグローバルIPアドレスを取得する対象になります。また、LAN側からインターネットへ出る通信の送信元IPアドレスをこのインターフェースのグローバルIPに変換する**NAT(NAPT)**は、原則としてこのWANインターフェースを境に適用されます。ステートフルファイアウォール機能も、多くの家庭用ルーターでは「WAN側から新規に始まる通信は原則遮断、LAN側から始まった通信の戻りは許可」という非対称なデフォルトポリシーで動作します。デフォルトルート(0.0.0.0/0の行き先)も通常このインターフェースを指します。
