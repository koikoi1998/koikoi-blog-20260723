...
<details>
<summary>ルーターの「WANポート」「LANポート」で内部処理は変わるのか</summary>

家庭用・SOHO向けルーターの背面には、たいてい「WAN」と書かれたポートが1つ、「LAN」と書かれたポートが複数並んでいます。この2種類は物理的な見た目が同じでも、ルーター内部では明確に異なる扱いを受けています。

- **WANポート**: ルーター内部で「WAN側インターフェース」として扱われ、ISPからDHCPまたはPPPoEでグローバルIPアドレスを取得する対象になります。また、LAN側からインターネットへ出る通信の送信元IPアドレスをこのインターフェースのグローバルIPに変換する<strong>NAT(NAPT)</strong>は、原則としてこのWANインターフェースを境に適用されます。ステートフルファイアウォール機能も、多くの家庭用ルーターでは「WAN側から新規に始まる通信は原則遮断、LAN側から始まった通信の戻りは許可」という非対称なデフォルトポリシーで動作します。デフォルトルート(0.0.0.0/0の行き先)も通常このインターフェースを指します。
- **LANポート**: 内部では1つの「LAN側インターフェース(ブリッジ)」に束ねられていることが多く、家庭用ルーターの複数のLANポートは実質的に内蔵の小さなL2スイッチを通じて1つのセグメントに接続されています。DHCPサーバー機能はこのLAN側インターフェースに対して動作し、プライベートIPアドレスを配布します。

つまり「WAN/LAN」という呼び分けは、物理的なポートの性能差ではなく、**そのポートがルーター内部でどちらの論理インターフェース(ゾーン)に割り当てられているか、そしてそこにNAT・ファイアウォールのデフォルトポリシー・DHCPサーバーといったどの機能が紐づいているか**という設定上の違いを表しています。実際、多くの業務用ルーターやL3スイッチでは、どの物理ポートをWAN(アップリンク)側にするか、LAN側にするかは工場出荷時に固定されておらず、管理者が設定で自由に割り当てられます。家庭用ルーターでポートが固定的に見えるのは、内部の回路・ファームウェアがコスト最適化のためにWANポート1つ分だけを独立した物理インターフェースとして持ち、残りのLANポート群をあらかじめ内蔵スイッチでまとめてしまっているためです。
