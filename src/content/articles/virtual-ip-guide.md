...
### 実IPは省略できないのか、「稼働系」はどこで・どうやって決まるのか

ここまでの説明を読むと、「稼働系サーバーが代表IPをそのまま持つのであれば、そもそも実IPは不要で、代表IPだけで運用できるのではないか」という疑問が浮かぶかもしれません。実務上、実IPを省略することはまずありません。理由は主に2つあります。

- **代表IPはいつ何時、別のサーバーへ移動するかわからない**からです。稼働系サーバー自身の管理(SSH/RDPでのログイン、監視エージェントからの死活監視、パッチ適用など)を代表IP宛に行ってしまうと、フェイルオーバーが起きた瞬間に管理対象が別の物理サーバーへ切り替わってしまい、「今どの物理サーバーにアクセスしているのか」が分からなくなります。実IPは、代表IPの状態にかかわらず常にその物理サーバー自身を指す、固定された宛先として使われます。
- **クラスタリングソフトウェア同士の死活監視(ハートビート通信)には、代表IPではなく実IPが使われる**からです。稼働系・待機系のどちらが今生きているかを判断するために、各サーバーは互いの実IP宛に定期的な生存確認パケットを送り合っており、この判定の基盤となる通信経路自体が、切り替わる可能性のある代表IPに依存してしまっては本末転倒です。

「稼働系」の判定自体は、外部の管理サーバーや中央のデータベースが行っているわけではなく、 **クラスタを構成する各サーバー自身が、このハートビート通信によって分散的に(自律的に)決定しています。** VRRPであれば各ルーターが持つ「優先度」の比較によるマスター選出、Windows Server Failover Clusteringであればクラスタの過半数(クォーラム)を保った状態でのリソースの所有権管理、というように実装ごとに具体的な選出アルゴリズムは異なりますが、いずれも特定の1台がすべてを管理する中央集権的な仕組みではなく、 **参加サーバー同士が対等にメッセージを交換し合意形成する、分散システムとしての設計** になっている点は共通しています。

<details>
<summary>コールドスタンバイ(待機系を平常時シャットダウンしておく構成)の場合はどうなるのか</summary>

待機系サーバーの電源そのものを平常時は落としておく「コールドスタンバイ」構成では、上記のハートビート通信自体が待機系側で動いていないため、VRRP/HSRPのような自律的な即時フェイルオーバーは成立しません。この場合、代表IPの切り替えは、クラスタリングソフトウェア同士の自律的な合意ではなく、**外部の監視システム(あるいは運用担当者)が稼働系の停止を検知し、待機系サーバーを起動させる**という、一段外側のオーケストレーション層に委ねられます。待機系サーバーは、実IP・代表IPの両方を割り当てる設定自体はあらかじめ済ませておき、電源が入っていない間はその設定が単に有効化されていないだけの状態です。起動後にOSとクラスタリングソフトウェアが立ち上がった時点で、実IPと代表IPの双方が同時にそのNICへバインドされ、Gratuitous ARPが送信されます。つまりこの場合も、代表IPというデータそのものをサーバー間で受け渡しているわけではなく、どちらのサーバーがそのIPを今バインドしているかという状態が切り替わる、という基本構造自体はホットスタンバイと変わりません。
...
スイッチ非依存モードにおいて「どちらのNICがアクティブか」を決めているのは、VRRP/HSRPのように他のサーバーとネットワーク越しに合意形成するプロトコルではなく、 **そのサーバー自身のOS・NICドライバ(チーミングドライバ)が、自分自身のリンク状態(ケーブル抜線やリンクダウンの検知)だけを根拠にローカルに判断している**、という点が代表IPのIPテイクオーバーとの決定的な違いです。あらかじめ「優先アダプター(Primary/Preferred Adapter)」を設定しておけば平常時はそれが優先的にアクティブへ選ばれますが、これは「正常」「異常」という区別があるわけではなく、あくまで管理者が設定した優先順位と、実際にリンクアップしているかどうかの2つだけを根拠に、チーミングドライバが機械的に決定しています。IPテイクオーバーが「複数の別々のサーバー同士が、ネットワーク越しにやり取りして代表権を決める」のに対し、スイッチ非依存モードのチーミングは「1台のサーバー内部だけで完結する、ローカルな切り替え判断」である、というスコープの違いを押さえておくと混同しにくくなります。
...
ここで区別しておきたいのは、代表IPが使っているのは あくまで **同じ物理NICに複数のIPアドレスを持たせる「IPエイリアス」** であり、これは「1台のサーバーの中に複数の(物理・仮想)ネットワークインターフェースを用意し、それぞれに専用のIPアドレスを持たせる」構成とは別の仕組みだという点です。たとえばVPNサーバーがクライアントに払い出すゲートウェイ用のIPアドレスは、既存の物理NICにIPエイリアスとして追加されるのではなく、OSが動的に作成する専用の仮想ネットワークインターフェース(点対点リンクの片端)に割り当てられる、後者の実例です。どちらも「1台のサーバーが複数のIPアドレスを同時に扱う」という結果は共通していますが、前者(IPエイリアス)は同じL2セグメント上の別名として振る舞うのに対し、後者(専用インターフェース)はそもそも別のリンク(あるいは別のセグメント)への入り口として振る舞う、という役割の違いがあります。

### なぜ物理NICに「元から」割り当てられていないIPでアプリケーションにアクセスできるのか

前項で見た「1枚の物理NICに複数のIPアドレスを同時に割り当てられる(IPエイリアス)」という性質は、代表IPのようなクラスタ用途に限った話ではありません。**Webサーバーやアプリケーションを、NICが最初に持っていた「メイン」のIPアドレスとは別のIPアドレスで公開したい**場合にも、そのまま応用されています。IPアドレスは物理的な配線そのものではなく、あくまでOSがそのネットワークインターフェースに対して設定する論理的な値であり、アプリケーション(Webサーバーなど)は、**OSがそのホストのインターフェースに対して「これは自分のIPアドレスだ」と認識してさえいれば**、それがNIC本来の「1つ目」のIPかどうかに関わらず、任意のIPアドレスにソケットをバインドしてリクエストを待ち受けられます。

ここでいう「OSが認識しているローカルIPアドレス」という条件に、192.168.x.xのような特定のアドレス帯であるという制約はありません。プライベートIPでもグローバル(パブリック)IPでも、そのホストのいずれかのネットワークインターフェースに実際に設定されてさえいれば、OSはそれを自分のIPアドレスとして認識します。逆に、どのインターフェースにも設定されていない(=OSがまだ知らない)IPアドレスへソケットをバインドしようとすると、OSはこれを拒否します(Linuxでは`EADDRNOTAVAIL`、Windowsでは`WSAEADDRNOTAVAIL`に相当するエラーになります)。したがって、これから使いたいIPアドレスがあれば、サーバー構築時であれ後からであれ、まずそのIPアドレスをOS側のネットワーク設定(NICへの追加IPの割り当てなど)として明示的に登録しておく必要があり、これが「OSが認識している」状態を作る唯一の方法です。後から追加したセカンダリIPや代表IP宛のリクエストであっても、同じ1枚の物理NICが受信し、OSのTCP/IPスタックが宛先IPアドレスに応じて正しいソケット(アプリケーション)へ振り分ける、という形で成立します。
