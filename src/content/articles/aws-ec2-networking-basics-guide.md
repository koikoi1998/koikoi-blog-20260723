...
.pemと.ppkは、あくまで同じ秘密鍵情報を格納する<strong>ファイル形式(コンテナ)</strong>の違いであり、鍵自体が使っている暗号アルゴリズム(RSA、ED25519など)や、鍵としての安全性そのものに違いはありません。PuTTYgenでの変換は、単に同じ鍵情報を異なる形式へ再エンコードしているだけの操作です。

</details>

<details>
<summary>なぜ昔はPuTTYが主流で、今はOpenSSHが標準になったのか</summary>

Windowsには長らく、Linux・macOSのようなSSHクライアントが標準搭載されていませんでした。この空白を埋めるために、1990年代末に登場したPuTTYが、無料で軽量、GUI操作もできるという理由でWindows利用者の事実上の標準ツールとして広く普及しました。この状況が変わったのは比較的最近で、**Windows 10(2018年のFall Creators Update)およびWindows Server 2019から、OpenSSHクライアントがオプション機能として標準搭載されるようになりました。** これにより、わざわざ別のソフトウェアをインストールしなくても、コマンドプロンプトやPowerShellから直接`ssh`コマンドが使えるようになっています。現在新しくSSH接続の手段を選ぶのであれば、追加のインストールが不要で、Linux・macOSと同じコマンド体系で扱えるOpenSSHを選ぶ方が合理的です。PuTTYは、既存の運用で使い続けている場合や、GUIでの細かい接続設定(文字コード、キーバインドなど)を好む場合に選ばれています。
...
<details>
<summary>「VPCのルーター」はVPC全体で1つなのか、サブネットごとに1つなのか</summary>

**予約される`X.X.X.1`は、サブネットごとに1つずつ存在します。** VPC全体にただ1台の物理的・仮想的なルーター機器が存在して、それを全サブネットが共有しているわけではありません。オンプレミスのネットワーク機器とは発想が異なり、AWSのVPCルーティングは、各サブネットに紐づく**ルートテーブル**という設定情報に基づいて、AWS基盤側が透過的に(実体としてのルーター機器を意識させずに)経路制御を行う、完全に仮想化・分散化された仕組みです。「サブネットごとに、そのサブネット専用のゲートウェイアドレスとして`X.X.X.1`が用意されている」とイメージする方が実態に近く、各サブネットのルートテーブルを個別に設定・確認できるのも、この「サブネットごとに独立した経路制御」という設計を反映しています。
