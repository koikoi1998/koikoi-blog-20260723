...
<details>
<summary>`~`(チルダ)は何を指しているのか</summary>

シェル上でよく見る`~`は、**「現在ログインしているユーザー自身のホームディレクトリ」を指す特殊な記号**です。シェルが実行時に、ログイン中のユーザー名に応じて、`/home/<ユーザー名>`(一般ユーザーの場合)や`/root`(rootユーザーの場合)へ自動的に展開してくれます。たとえば一般ユーザー`ubuntu`で`cd ~`と入力すると`/home/ubuntu`へ、`sudo su -`で切り替えた`root`で`cd ~`と入力すると`/root`へ、それぞれ移動します。`~user`のように別のユーザー名を続けると、そのユーザーのホームディレクトリを指すこともできます(`~ubuntu`なら常に`/home/ubuntu`)。このハンズオンで頻出する`~/.ssh/authorized_keys`のようなパスは、「自分のホームディレクトリ配下の`.ssh`ディレクトリ」を指しており、実行するユーザーによって実体のパスが変わる、ということを意識しておいてください。
