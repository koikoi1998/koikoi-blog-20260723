...
- **ランタイム・ライブラリ・SDK・APIという用語の全体像**: これらの用語そのものの一般的な整理は[フレームワークとは何か](/articles/software-framework-guide)で扱っています。本記事では、その中の「ランタイム(実行エンジン)」に焦点を絞り、.NET Frameworkという具体例でさらに掘り下げます。
...
### 「実行エンジン(ランタイム)」とは、具体的にどういうものなのか

ここが最もつまずきやすいポイントなので、ソフトウェア・アプリケーションとの違いを、具体例で徹底的に切り分けます。

**普段「アプリケーション」と呼んでいるもの(`notepad.exe`のようなファイル)の多くは、そのCPUがそのまま実行できる機械語の命令(マシンコード)そのものが書き込まれたファイルです。** Windowsは、このファイルをメモリに読み込み、CPUに「ここから実行して」と指示するだけで、あとはCPUがファイルの中身を直接解釈して実行します。ここには、OS以外の特別な「実行エンジン」は必要ありません。

**ところが、.NETベースのアプリケーション(PowerShellのcmdletや、.NETで書かれたコンパイル済みのプログラム)は、事情がまったく異なります。** これらのファイルの中に書き込まれているのは、CPUがそのまま実行できるマシンコードではなく、**IL**(中間言語、Intermediate Language)と呼ばれる、CPUの種類に依存しない、いわば「半完成品」のコードです。ILは、そのままではCPUに何もさせることができません。ここで初めて**CLR**(共通言語ランタイム)というランタイム(実行エンジン)の出番になります。CLRは、実行されようとしているILを、その場でその機械(そのCPU)向けの本物のマシンコードへと**その都度変換(JITコンパイル、Just-In-Timeコンパイル)しながら実行していく**、という特別な仕事を担っています。

つまり、**「アプリケーション」がファイルの中身(何を実行したいか、という設計図)だとすれば、「実行エンジン(ランタイム)」は、その設計図を読み、その場でCPUに実行させられる形に翻訳しながら現場を仕切る職人**にあたります。.NETベースのプログラムは、CLRという職人がその場にいない限り、CPUに対して文字通り何も実行させることができません。これが、「.NET Frameworkという実行エンジンが、Windows上に別途インストール・有効化されている必要がある」理由の核心です。ライブラリ(クラスライブラリ)は、この職人(CLR)が仕事をする際に使う、あらかじめ用意された道具箱のようなものであり、実行エンジンそのものとは役割が異なります。コマンドラインシェル(PowerShellやcmd.exe)は、これらすべてを土台にして、人間がコマンドを打ち込んで操作するための、いわば「窓口」の役割を果たすアプリケーションの1つです。
