...
**そもそも「回線」とは**: 送信元と宛先の間でデータをやり取りするための物理的・論理的な「通り道」を指す言葉です。電話回線であれば銅線や光ファイバーといった伝送路そのものを指し、後述のIPネットワークの文脈で使うときは、パケットが実際にたどる経路（ルート）全体を指すことが多く、指し示す実体は文脈によって変わりますが、「データが通る道」という点は共通しています。

**電話回線とIPネットワークの違い**: 電話回線は、通話中は相手との間に1本の物理回線を占有し続ける「回線交換」方式です。ダイヤルアップ接続とは、この電話回線を使って自宅のモデム（modulator-demodulator、変復調装置。コンピューターが扱うデジタル信号と、電話回線を流れるアナログの音声信号とを相互に変換する機器で、送信側で変換したアナログ信号を受信側のモデムが再びデジタル信号に戻すことで、本来デジタル通信用ではない電話回線の上でデータ通信を成立させます）からISP（後述）のモデムへ電話をかけ、その占有された1本の回線の上でPPPのやり取り（LCP→認証→IPCP）を行うことで、電話回線を「1対1のデータ通信リンク」として使う仕組みです。一方、現在主流のIPネットワークは、1本の回線を多数の通信が共有し、データをパケット単位に分割して宛先ごとに振り分ける「パケット交換」方式です。相手を占有しない代わりに、宛先を識別する仕組み（IPアドレスとルーティング）が別途必要になる、という違いがあります。

この違いが実際にPPPにとって何を意味するのか、もう少し具体的に見てみます。回線交換方式の電話回線では、通信中は相手との間に他の誰の通信も割り込まない1本の物理回線が占有され続けるため、「今この回線を流れているデータの相手は誰か」を毎回明示する必要がありません——回線そのものが相手を一意に決めているからです。また、データは1本の物理的な経路をそのまま流れるだけなので、送った順番通りに届くことも（物理層のエラーを除けば）前提にできます。PPPが「宛先アドレスを持たず、届いたフレームをただ順番に処理する」というシンプルな設計で成立しているのは、この「1対1で専有され、順序も保たれる回線」という前提の上に乗っているからです。

一方パケット交換方式のIPネットワークでは、1本の物理回線や中継するルーターを無数の通信が同時に共有しています。そのため送り出したデータの宛先を、パケット1つ1つにIPアドレスとして明示しなければ、途中のルーターはどこへ転送すればよいか判断できません。さらに各ルーターはパケットを受け取るたびに独立して次の転送先を決めるため、同じ宛先へ送った複数のパケットが必ずしも同じ経路を通るとは限らず、途中の混雑状況によって遅延・順序の入れ替わり・消失が起こり得ます。電話回線が「専有・順序保証」を前提にできたのに対し、IPネットワークは「共有・順序不定」を前提にせざるを得ない、という点が本質的な違いです。

ここで自然に浮かぶのが、「電話回線が1対1の専用線だとしても、実際には発信元の交換局からISPまで、複数の交換機（スイッチ）を経由しているのではないか」という疑問です。これは正しい疑問で、固定電話網の内部では、最寄りの交換局からISPまでの間に複数の交換機を経由するのが普通です。ポイントは、経由する交換機の**数**ではなく、**その経路が呼（通話）の間ずっと固定され、他の通信に横取りされない**という性質です。伝統的な電話網では、経由する各区間が64kbpsのデジタル回線（タイムスロット）としてTDM（時分割多重）で管理されており、発信時に「この呼のためにこの区間のこのタイムスロットを使う」という割り当てが、経由するすべての交換機で行われ、通話中はその割り当てが変わらず維持され続けます。IPネットワークのルーターが、パケットを受け取るたびにその都度「次にどこへ転送するか」を独立に判断し直しているのとは対照的に、電話網の交換機は、呼が確立した瞬間に決まった経路をそのまま通話終了まで使い続けます。「複数の中継機器を経由する」こと自体は電話網もIPネットワークも同じですが、**経路が呼の間固定されるか、パケットごとに選び直されるか**という点が本質的な違いです。この「経路が呼の間ずっと固定される」性質があるからこそ、電話番号をダイヤルした時点で相手（この場合はISP）が確定してしまえば、以降のやり取りには宛先情報を一切含める必要がなくなります。PPPが宛先アドレスを持たない設計になっているのは、まさにこの前提の上に成り立っています。
...
**ダイヤルアップ接続の実際の手順とIPアドレスの関係**: 発信ボタンを押してからIPで通信できるようになるまでを時系列で並べると、次のようになります。
...
3. **PPPのLCP**: このシリアルリンクの上でPPPのLCPが動き始め、リンクパラメータと使用する認証方式を合意します。ここで初めてL2的な「フレーム」の概念が成立します。
4. **PPP認証フェーズ**: 合意した方式（PAP/CHAPなど）に従い、ユーザー名とパスワードが**モデム間のデジタル信号として自動的に**やり取りされます。着信はデータ着信として扱われるため、人間のオペレーターが応対して口頭でID・パスワードを聞き取る、という場面はありません。ここでいう「モデム間のデジタル信号」とは、ステップ2のモデムハンドシェイクによって回線がすでに音声用途からデータ通信用途に切り替わった後の、モデムが変調した連続的なデジタルデータそのものを指します。特別な音声認識や人間の聞き取りを介するわけではなく、この後IPCP完了後にWebページやメールをやり取りするのと同じ「モデムのデータ回線」の上に、たまたまLCP→認証→IPCPというPPPの制御データが最初に流れているだけです。この時間帯の回線を人間が耳で聞いても、意味の分かる言葉や音声ではなく、モデム特有の連続したノイズにしか聞こえません。
5. **IPCP**: 認証が成功して初めて、ISP側がクライアントにIPアドレスを1つ払い出します。**この瞬間まで、クライアント側には一切IPアドレスがありません。**

ここで払い出されたIPアドレスは、以降そのPPPリンク上を流れる全IPパケットの送信元アドレスとして使われます。「ISPのモデムを通してインターネット接続する時点ですでにIPネットワークになっている」という理解は、実は前後関係が逆です——電話回線という物理的な土管そのものはIPネットワークではなく、PPP（のIPCP）によってIPアドレスが払い出された**瞬間から**、そのリンクがIPネットワークの一部として機能し始めます。IPCPは「すでにあるIPネットワークに接続するための手続き」ではなく、**「その1対1のリンクをIPネットワークの一部に昇格させる手続き」そのもの**です。

なお、電話番号を指定する行為はL1レベルの回線確保であり、L2のフレーミングを行っているわけではありません（L2相当のフレーミングは、この後動くPPPのLCPが担います）。1本のモデム回線は同時に1通話しか受けられないため、話し中の間は同じ番号に他の人がかけても繋がりませんが、ISPは通常、代表番号の背後に**多数のモデムを収容したモデムプール**を持ち、空いているモデムに着信を自動的に割り振ります。したがって「1人が接続している間は他の誰もそのISPを使えない」わけではなく、モデムプールの空き回線数だけ同時接続が可能だった、というのが実態です。

ここまでの手順は、あくまで「電話回線を使ってデータ通信（インターネット接続）をしたい」場合の話である点にも注意してください。単なる音声通話（普通の電話）であれば、モデムハンドシェイクもPPPのLCP・認証・IPCPも一切発生しません。回線が繋がった後は、送受話器が拾ったアナログの音声信号がそのまま相手に伝わるだけです。PPP（RFC 1661）自体も、「電話をかけるために発明された」規格ではありません。PPPは**2点間を直接つなぐシリアルリンク全般（電話回線によるダイヤルアップに限らず、専用線など他のポイントツーポイント回線も含む）の上で、リンクの確立・認証・IPアドレス設定を行うための汎用プロトコル**として設計されています。電話回線は、当時安価に入手できた「2点間のシリアルリンク」の実現手段の1つとして使われたに過ぎません。つまり「電話をかける」という行為自体にPPPは一切必須ではなく、PPPが必要になるのは、その電話回線を使って**データ通信（IPネットワークへの接続）を行いたい場合に限られる**、というのが正確な理解です。
...
NAT（NAPT）デバイスは、内部の複数端末で1つのグローバルIPを共有するために、「プロトコル種別＋送信元IPアドレス＋送信元ポート番号＋宛先IPアドレス＋宛先ポート番号」の組み合わせを変換テーブルのキーとして管理し、戻ってきたパケットをどの内部端末に転送すればよいかを判断しています。ところがESP自体にはポート番号の概念がありません。そのため、クライアントが家庭用ルーターなどのNAT配下にいる場合、次の2つの問題が起きます。1つ目は、多くの安価なNAT機器がそもそもIPプロトコル番号50（ESP）のパススルーに対応しておらず、ESPパケットを単純に破棄してしまうことです。2つ目は、パススルーに対応した機器であっても、変換テーブルのキーに使えるポート番号がESPには存在しないため、複数の内部端末が同時にESP通信をしようとした場合にどちらの端末宛かを区別できないことです。これを解決するのが**NAT-T（NAT Traversal、RFC 3947/3948）**です。

1. **NATの検出**: IKEフェーズ1の中で、双方が「送信元IPアドレス+ポート番号のハッシュ値（NAT-Dペイロード）」を計算して交換します。相手が計算したハッシュ値と、自分が実際に受信したパケットの送信元情報から計算したハッシュ値が一致しなければ、途中でNATによってアドレス・ポートが書き換えられたと判断できます。ここで注意したいのは、**NAT-Dが検出しているのはあくまで「経路上でアドレス・ポートが書き換えられたという事実」であり、それが正規のNAT機器によるものか、悪意ある第三者による改ざん・なりすましかまでは区別できない**という点です。NAT-Dはあくまで「この後UDP4500にフロートすべきかどうか」を判断するための機能的な仕組みであり、セキュリティ上の防御機構ではありません。実際になりすまし・改ざんを防いでいるのは、前述の「IKEフェーズ1の認証」で解説した**PSKまたは証明書によるハンドシェイクの署名/ハッシュ検証**です。DH鍵交換とnonceを含めて認証しているため、途中経路でIPアドレスやポート番号が（NATであれ攻撃者であれ）書き換えられても、対応する鍵材料を持たない相手はこの認証を通過できません。さらにフェーズ2以降は、後述のESPのICV（Integrity Check Value）によってパケットごとの改ざん検知も行われます。つまり「接続予定のないIPや偽装されたIPだった場合にどうなるか」への答えは、**NAT-Dの一致・不一致にかかわらず、後続の認証パートとESPの完全性検証が正規の通信であることを別途保証している**、ということになります。
2. **UDPへのカプセル化**: NATが検出されると、以後のIKEとESPのやり取りはすべて**UDPポート4500**にフロート（切り替え）します。ESPパケットはUDPヘッダーでさらに包まれ（RFC 3948）、NATデバイスは通常のUDPパケットとして送信元・宛先ポートに基づくNAPT変換ができるようになります。同じUDPポート4500の上をIKEメッセージとカプセル化されたESPパケットの両方が流れることになるため、受信側はこの2つを区別する必要があります。これを可能にしているのが**Non-ESPマーカー**です。IKEメッセージの場合はUDPペイロードの先頭に4バイトの`0x00000000`（Non-ESPマーカー）を付けてから通常のIKEメッセージ（ISAKMPヘッダー）を続け、ESPパケットの場合はこのマーカーを付けずESPヘッダーをそのまま続けます。受信側はUDPペイロードの先頭4バイトを見るだけで、どちらの種類のメッセージかを瞬時に判別できます。**逆にNATが検出されなかった場合**は、この切り替えは起きません。IKEはそのままUDPポート500で継続し、ESPもUDPに包まれることなく**IPプロトコル番号50のパケットとして直接**送受信されます。UDPで包む分のヘッダーオーバーヘッド（8バイト＋Non-ESPマーカー4バイト）が発生せず、後述のキープアライブも不要になるため、NAT-Tはあくまで「NAT配下でも通信できるようにするための迂回策」であって、NATがない環境ではむしろ使われない、という位置づけです。
3. **キープアライブ**: 家庭用ルーターなどのNAPT変換テーブルは、無通信状態が一定時間（実装によりますが数十秒〜数分程度）続くとエントリを削除してしまいます。ESP/IKEのやり取りが常時発生するとは限らないため、多くの実装は20〜30秒間隔でペイロード1バイトだけのNAT-Tキープアライブパケットを送り、NAT変換テーブルのエントリが失効してセッションが切断されるのを防ぎます。NAT配下にいない場合はNAT変換テーブル自体が関与しないため、このキープアライブも送信されません。
...
なお、IKEフェーズ1で使われるDiffie-Hellman鍵交換の数学的な仕組みや、証明書認証で使われる公開鍵暗号・デジタル署名・証明書チェーンの検証といったPKIの仕組みそのものは、別記事「[PKIとデジタル証明書の仕組みを『上位1%』の視点で理解する](/articles/pki-guide)」でさらに深掘りしています。
