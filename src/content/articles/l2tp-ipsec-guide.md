...
1つ目は、**ユーザー空間の通常のソケットAPIだけで実装できる**ことです。ESP(IPプロトコル番号50)のような「TCP/UDP以外のIPプロトコル」を直接扱うには、多くのOSでは生ソケット(raw socket)の作成にroot権限相当の特権が必要です。一方UDPは、`bind()`して特定のポート(L2TPなら1701番)を待ち受けるだけの、一般的なアプリケーションプログラマにとってごく普通のソケットプログラミングで実装できます。実際、Linux環境でよく使われるL2TP実装の`xl2tpd`は、特権を持たない通常のデーモンプロセスとしてUDPソケットを扱うだけで動作しています。デーモンとしての実装がなぜユーザー空間の通常プロセスで完結できるのかは、[デーモン(daemon)とは何か——Linuxのバックグラウンドプロセスを『上位1%』の視点で理解する](/articles/linux-daemon-guide)で扱っています。

2つ目は、**ポート番号による多重化**です。IPプロトコル番号(ESPなら50)は、1つのIPアドレスに対して基本的に1つのプロトコルとして扱われ、OSカーネルもプロトコル番号単位でしか処理を振り分けられません。UDPというトランスポート層を挟むことで、同じIPアドレス上で複数のUDPポート(L2TPの1701番、IKEの500番、NAT-Tの4500番など)を独立したサービスとして同時に待ち受けることができます。
