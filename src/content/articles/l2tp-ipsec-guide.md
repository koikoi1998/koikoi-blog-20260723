...
ポイントは、**ESPが「UDPヘッダー（ポート1701）ごと」暗号化する**ことです（ESP自体の詳しい仕組みは後述の「IPsecが担当する役割」で解説します）。図中の「元のデータ（内側のIPパケット）」もUDP/L2TP/PPPと一緒に丸ごと暗号化対象に含まれるため、外側から見える情報は「送信元・宛先のグローバルIPアドレス」と「これがESP（IPプロトコル番号50）のパケットである」ということだけであり、UDPポート1701はもちろん、内側のIPパケットが持つ送信元（仮想IP）・宛先（社内サーバーのIP）というアドレス情報自体も経路上の機器からは見えません。ここで注意したいのは、**この方式が「トランスポートモード」と呼ばれるのは「ESPが新たな外側IPヘッダーを重ねて付与しない」という意味であって、「内側の元のIPパケットが平文のまま残る」という意味ではない**という点です（この誤解については、後述の「よくある誤解」の誤解4でさらに詳しく扱います）。
...
<details>
<summary>トンネルの仕組みをもう一段掘り下げる(専用リンクの錯覚が成立する理由・よくある疑問2つ)</summary>

**なぜ「包み込む」だけで専用リンクの錯覚が成立するのか。** 実装上、PPPは「フレームを送受信する下位層」を抽象化されたインターフェースとしてしか意識しません——電話回線のシリアルポートであろうと、IPネットワーク上のトンネルであろうと、PPP自身は「フレームを渡せば相手に届き、相手からフレームが届けば受け取れる」という送受信の契約さえ守られていれば、その下がどんな媒体かを一切気にしない設計になっています。L2TPは、この下位層のインターフェースを、物理的なシリアル回線の代わりにIPネットワーク上で実装したものです。具体的には、PPPが送り出した1つのフレームを、そのままUDP/IPのペイロードとして包み、宛先はあらかじめ確立済みの**Tunnel ID・Session ID(後述)に対応する1台の相手に固定**して送り出します。つまりPPPからすれば、「フレームを渡せば、決まった1人の相手にだけ届く」という専用リンクとまったく同じ契約が、実装の中身がシリアルポートからL2TPの送受信処理に置き換わっただけで、そのまま維持され続けます。

この整理を踏まえると、2つのよくある疑問にも明確に答えられます。**1つ目「PPPのやり取りを暗号化するためにトンネルを作るのか」** ——答えは「いいえ」です。L2TP自体には暗号化機能が一切なく(後述の「よくある誤解」でも扱います)、トンネルの目的はあくまで「PPPが要求する1対1のリンクを、共有されたIPネットワークの上でエミュレートすること」であり、暗号化とは無関係です。暗号化は完全に別レイヤーであるIPsec(ESP、後述の「IPsecが担当する役割」で解説します)の仕事です。

**2つ目「IPsecですでに暗号化されているのに、なぜPPPネゴシエーションをいきなり始めず、L2TPのトンネル化という手順を挟む必要があるのか」** ——ここで効いてくるのが、IPsec(ESP)が提供するものとPPPが必要とするものの違いです。IPsec(ESP)が提供するのは「2つのIPアドレスの間の、秘匿されたIP通信路」であって、その中に「PPPが動作できる1対1のリンクの形」を用意してくれるわけではありません。PPPはあくまで「専用リンクの上で1つの相手とだけLCP/認証/IPCPをやり取りする」ことを前提に設計されたプロトコルであり、IPsecが暗号化したIPパケットの流れに直接乗れるようにはできていません。L2TPが挟まることで、暗号化済みの経路の中に、さらに前段の「PPPが動作できる仮想リンク」が用意されるのです。加えて、L2TPのTunnel ID/Session IDは、1台のVPNサーバーに同時多数のユーザーが接続する際、それぞれのPPPセッションを識別・多重化する役割も担っており、これはIPsecのSA(Security Association)だけでは代替できない機能です。

</details>

L2TPがこのトンネリングに実際に使うのは**UDP/IPというカプセル化**です(この設計はIPsecと組み合わせる以前のL2TP単体の仕様(RFC 2661)ですでに決まっています)。ESP単体では足りずわざわざUDPを挟む理由、そして受信側は実際にどうやってこの何層にも重なったヘッダーを正しく剥がしているのかという、より掘り下げた疑問には、次の折りたたみで答えます。

<details>
<summary>なぜL2TPはUDPを選んだのか、受信側はどう非カプセル化しているのか</summary>

**ESPはIPプロトコル番号50として直接IPヘッダーの後ろに続く形で暗号化・認証を行っており、それだけでVPNサーバーまでパケットを届けられるように見えるのに、なぜわざわざUDPヘッダーを挟む必要があるのか**、という疑問がまず浮かびます。ESPによる暗号化は後から重ねられる別レイヤーの話であり、「ESPがあるからUDPは不要になる」という関係にはそもそもありません。L2TP自体がUDPを選んだ理由は主に2つあります。

1つ目は、**ユーザー空間の通常のソケットAPIだけで実装できる**ことです。噛み砕いて言うと、ESP(IPプロトコル番号50)のような「TCP/UDP以外のIPプロトコル」を直接扱うのは、OSに対して「特別な権限をください」と申請しないと使えない特別な窓口を通るようなものです(多くのOSでは生ソケット(raw socket)の作成にroot権限相当の特権が必要です)。一方UDPは、`bind()`して特定のポート(L2TPなら1701番)を待ち受けるだけの、いわば「誰でも使える普通の窓口」であり、一般的なアプリケーションプログラマにとってごく普通のソケットプログラミングで実装できます。L2TPは元々、専用ハードウェアではなく普通のサーバー上で動く一般的なソフトウェアとして実装されることを想定していたため、この「特別な権限を必要としない」という点が設計上重視されました。実際、Linux環境でよく使われるL2TP実装の`xl2tpd`は、特権を持たない通常のデーモンプロセスとしてUDPソケットを扱うだけで動作しています。デーモンとしての実装がなぜユーザー空間の通常プロセスで完結できるのかは、[デーモン(daemon)とは何か——Linuxのバックグラウンドプロセスを『上位1%』の視点で理解する](/articles/linux-daemon-guide)で扱っています。

2つ目は、**ポート番号による多重化**です。直感的には、IPアドレスを「建物の住所」、IPプロトコル番号を「その建物に置ける郵便受けの種類」に例えると分かりやすくなります。ESPのようにIPプロトコル番号だけを頼りに通信する仕組みは、1つの建物(IPアドレス)につき「ESP用の郵便受け」を1つしか置けないようなものです。ところがVPNサーバーという1つの建物の中では、IKE(鍵交換)・L2TP(トンネル)・NAT-T(NAT越え)という複数の別々の「サービス」が同時に動いている必要があります。もしL2TPもESPのようにプロトコル番号だけで通信する設計だったら、IKEやNAT-Tと同じ建物(同じIPアドレス)に共存できません。UDPというトランスポート層を挟むと、この建物に「部屋番号」に相当するポート番号を割り振れるようになり、L2TPは1701号室、IKEは500号室、NAT-Tは4500号室というように、同じ住所(IPアドレス)の中で複数のサービスを独立して同時に待ち受けさせられます。これが「ポート番号による多重化」の意味です。より専門的な、プロトコル番号とポート番号がそもそもどう違うのか、なぜESPには後からポート番号を割り当てられないのかという掘り下げは、別記事「[TCP/UDPの「セッション」とポート番号の関係を『上位1%』の視点で理解する](/articles/tcp-udp-session-port-guide)」で徹底的に扱っています。

なお実際の通信では、この「L2TPがUDP1701を使う」という部分自体、後から重なるESPによってUDPヘッダーごと暗号化されるため、経路上の第三者やNAT機器からはUDPポート1701という情報自体が見えなくなります(前述の「誤解3」で扱った通りです)。L2TPがUDPを使う理由とIPsecがそれを保護する理由は、まったく別の設計判断が積み重なった結果に過ぎません。

**では受信側は、この何層にも重なったヘッダーを実際どうやって正しく剥がしているのでしょうか。** 先に結論を言うと、「L4(トランスポート層)のヘッダーを必ず挟まなければならない」というルールはOSI参照モデル自体には存在しません——その証拠がESPで、UDPやTCPのようなL4ヘッダーを一切持たずIPヘッダーの直後にプロトコル番号50として直接続くのに、問題なく運ばれ処理されます。**カプセル化の各層のヘッダーが本質的に果たしている役割は「次にこの中身をどの処理に渡せばよいかを示す識別子を運ぶこと」であり**、その識別子がIPヘッダーのプロトコル番号なのか、UDPヘッダーのポート番号なのか、L2TPヘッダーのTunnel ID/Session IDなのか、PPPヘッダーのProtocolフィールドなのかは、その層が解決すべき多重化の問題に応じて決まります。ESPのようにL4を挟まないプロトコルも、**SPI(Security Parameters Index)** という識別子をポート番号の代わりに使うことで、複数の相手を区別しています。

この視点で、受信側(VPNサーバー)が実際にヘッダーを剥がしていく手順を追うと、次のようになります。

1. **外側のIPヘッダーを見る**: **Protocolフィールド**の値`50`からESPだと分かり、OSはこのパケットをIPsec処理のコードに渡します。
2. **ESPヘッダーを見る**: 先頭の**SPI**から、同時接続中の複数クライアントのうちどのIPsec SA(鍵・暗号アルゴリズムの取り決め一式)で復号すべきかを特定し、復号するとUDPヘッダーが現れます。
3. **UDPヘッダーを見る**: 宛先ポート番号`1701`から、OSはこのペイロードをL2TPを処理するプロセス(`xl2tpd`など)に渡します。
4. **L2TPヘッダーを見る**: **Tunnel ID**と**Session ID**の組み合わせから、どのクライアントのどのPPPセッションに属するフレームかを特定します。
5. **PPPヘッダーを見る**: **Protocolフィールド**の値(実データなら`0x0021`、IPCPの制御メッセージなら`0x8021`など)から、実データのIPパケットか制御メッセージかを判別します。
6. **最後に残るのが、内側の元のIPパケット**です。実データだと判別できた場合、PPP層はこれを普通のIPパケットとしてOSのルーティング処理に渡します。

つまり送信側が「データ→PPP→L2TP→UDP→ESP→外側IP」の順にヘッダーを重ねるのに対し、受信側はその**逆順で、1枚剥がすたびに識別子を読んで次の行き先を判断している**のが非カプセル化の実体です。層の数や種類に絶対的な決まりはなく、それぞれの層が抱える多重化の要件に応じた合理的な選択の結果である、と理解すると腑に落ちます。

</details>

### L2TPのメッセージ種別とTunnel ID/Session ID
...
<details>
<summary>IPCP完了後もPPPヘッダーが必要な理由</summary>
