...
- **この記事で得られること**: Windows Serverの役割として追加する<strong>IIS(Internet Information Services)</strong>と、その上で動く**ASP.NET**が、それぞれ何を担っているソフトウェアなのかを整理します。あわせて、IISマネージャーを開くと必ず表示される**Default Web Site**の正体、複数のWebサイトを1台のサーバーで運用する際に必要な**バインド設定**、そして**HTTP応答ヘッダー**の追加・変更の仕組みまでを体系的に理解します。
...
- **フレームワークとライブラリの違い**: 本記事では「ASP.NETはフレームワークである」という表現が登場しますが、「フレームワーク」「ライブラリ」といった用語そのものの違いは[ライブラリ(library)とは何か——静的リンク・動的リンクの仕組みを『上位1%』の視点で理解する](/articles/software-library-guide)や[フレームワークとは何か——ライブラリ・ランタイム・SDKとの違いを『上位1%』の視点で理解する](/articles/software-framework-guide)で整理しています。
...
### 「Webサイト」とは、正体としては何なのか

ここまでの説明を踏まえると、**「Webサイト」という言葉の正体は、HTML/CSSファイルの集まりのことではなく、「HTTPというプロトコルを解釈し、リクエストに応じてレスポンスを返すことができるソフトウェア(とその設定)」のこと**だと理解できます。IISが管理している「Webサイト」というエントリも、実体は「特定のバインド(IPアドレス・ポート・ホスト名)に届いたHTTPリクエストを受け取り、静的ファイルを返すか、ASP.NETなどの実行エンジンにレスポンス生成を委譲するか」を決める設定のかたまりです。HTML/CSSは、そのレスポンスとして返される**コンテンツの一形態**にすぎず、Webサイトという概念そのものとイコールではありません。

この「HTTPを処理できるソフトウェアであれば、それは(広い意味で)Webサーバーになりうる」という感覚は、実際に自分の手でごく小さなHTTPサーバーを書いてみると、驚くほどはっきりと実感できます。IISやNginxのような本格的な実装を使わなくても、**TCPソケットでリクエストの中身(1行目のメソッド・パス・バージョンと、ヘッダー)をテキストとして読み取り、決まった書式でレスポンスを書き返すだけ**で、ブラウザから開ける最小限の「Webサイト」が作れてしまいます。この体験は[自分の手でHTTPサーバーを書いてみる『上位1%』のハンズオン](/articles/minimal-http-server-handson-guide)で扱っています。
...
<details>
<summary>「Default Web Site」は「アプリケーションプール」なのか</summary>

紛らわしいのですが、**この2つは別々のオブジェクトです。** Default Web Siteは、上で説明した通り「バインド設定+物理パス」を持つ**サイト**であり、IISの役割をインストールすると、これとは**別に**`DefaultAppPool`という名前のアプリケーションプールも自動的に作成されます。既定の構成では、Default WebSiteという「サイト」が、DefaultAppPoolという「アプリケーションプール」に**割り当てられている**という関係にあり、この2つがどちらも「Default」という名前を持ったまま最初からペアで用意されているために、あたかも1つのものであるかのように混同されがちです。実務では、Default Web Siteのアプリケーションプールの割り当てを変更したり、複数のサイトを1つのアプリケーションプールに割り当てたりすることも自由にできます。

</details>

### バインド設定:1台のサーバーで複数のWebサイトを振り分ける仕組み

1台のIISサーバーで複数のWebサイトを同時にホストする場合、<strong>HTTP.sysが受信したリクエストを、どのWebサイトへ振り分けるべきかを判断するための設定が「バインド」</strong>です。バインドは、次の3つの要素の組み合わせで定義されます。
...
HTTPの場合、`Host`ヘッダーは暗号化されていないリクエストの中に含まれているため、単純にその値を見て振り分けられます。一方HTTPSの場合、TLSによる暗号化のハンドシェイクが完了する前に、どの証明書を提示すべきかを決める必要があるという課題があり、これは<strong>SNI(Server Name Indication)</strong>という、TLSハンドシェイクの初期段階でクライアントが希望するホスト名を平文で伝える拡張機能によって解決されています。現在のIISは、このSNIに対応しており、1つのIPアドレス・ポートで複数の異なる証明書を持つHTTPSサイトを同時にホストできます。

</details>

### Webサイトのホスト名と、DCのドメイン名に直接の関係はあるのか

結論として、**技術的な仕組みとしては直接の関係はありません。** バインドのホスト名は、あくまで「そのDNS名で名前解決してアクセスしてきたクライアントを、どのサイトへ振り分けるか」を決めるための値にすぎず、そのDNS名がAD DSのドメイン名(フォレスト・ドメインの名前空間)に属している必要はありません。公開Webサイトのホスト名がどのAD環境とも無関係であるのと同じ理屈です。

ただし、**社内向け(イントラネット)のサイトでは、両者が同じDNS名前空間を共有していることが多く、これが「関係がある」という印象につながりやすい**という実務上の事情があります。社内サイトのDNSレコードは、多くの場合すでに存在するAD統合DNS([AD環境のDNSはなぜこう設計されているのか](/articles/ad-dns-guide)を参照)にそのまま追加され、`intranet.corp.example.com`のように**AD DSのドメイン名を接尾辞として持つホスト名**が使われることが一般的です。これは、IISやHTTPが技術的に要求しているからではなく、**すでに存在する社内DNSインフラをそのまま再利用しているだけ**という、運用上の選択です。
...
- Default Web Siteは特別な予約された仕組みではなく、最初から用意されている通常のWebサイトエントリの1つです。Default Web Site(サイト)とDefaultAppPool(アプリケーションプール)は別々のオブジェクトで、既定でペアになっているだけです。
- バインド設定は、IPアドレス・ポート番号・ホスト名の組み合わせによって、1台のサーバー上の複数のWebサイトへリクエストを振り分ける仕組みです。ホスト名はDNS名にすぎず、AD DSのドメイン名との関係は技術的な要件ではなく、社内DNSインフラの再利用という運用上の選択です。
- 「Webサイト」の正体は、HTML/CSSファイルの集まりではなく、HTTPを解釈してレスポンスを返せるソフトウェア(とその設定)です。
