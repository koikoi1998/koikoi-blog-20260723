...
括弧の中に並んでいる5つの数値の前に、実は4つの項目が並んでいます。これらもまとめて理解しておきましょう。

| 項目 | 内容 |
|---|---|
| **`example.com.`** | このレコードが、どのゾーン(ドメイン)に関するものかを示す名前です。末尾のドット(`.`)は、DNSの世界での「ここが完全な絶対パスである」という意味の表記(FQDN)であり、省略記法ではありません。 |
| **`IN`** | レコードの「クラス」を表します。インターネットで使うレコードであることを示す`IN`(Internet)がほぼ常に指定され、実務でこれ以外の値を目にすることはまずありません。 |
| **`SOA`** | このレコードの「タイプ」です。A・NS・MXなどと同じ並びの、レコードタイプの一種であり、「これはSOAレコードですよ」と宣言しています。 |
| **`ns1.example.com.`** | このゾーンの**プライマリネームサーバー**(マスターサーバー)のホスト名です。 |
| **`admin.example.com.`** | このゾーンの**管理者のメールアドレス**を、DNSの表記ルールに従って書いたものです。メールアドレスの`@`をドット(`.`)に置き換えて表記する決まりになっているため、これは実際には`admin@example.com`という管理者アドレスを意味しています。 |

これら4つの項目に続けて、括弧の中に、次の5つの数値(タイマー系の設定値)が並びます。
