...
### NAT-D(NAT Detection)ペイロードの中身:なぜハッシュを2つ送るのか

前節の「1. NATの検出」を、もう一段掘り下げます。**NAT-D(NAT Detection、RFC 3947で定義されるペイロード種別)** は、IKEフェーズ1のメッセージ交換の中でやり取りされる、NATの有無を判定するための情報です。計算式は次の通りです。

```
NAT-D = HASH( ISAKMP Cookie(Initiator) | ISAKMP Cookie(Responder) | IPアドレス | ポート番号 )
```

ここでのCookie(ISAKMP SPIとも呼ばれます)は、IKEフェーズ1の最初のメッセージ交換で双方がそれぞれ生成する8バイトのランダム値で、ISAKMPヘッダーに含まれています。これをハッシュの材料に含めているのは、 **同じIPアドレス・ポートの組み合わせに対するハッシュ値が、別のセッションでも使い回されて第三者に予測・比較されることを防ぐため** です。セッションごとにCookieの値が変わるため、対象のIPアドレス・ポートが同じでもハッシュ値は毎回変わります。

**なぜ1回のやり取りでNAT-Dペイロードが2つ(ハッシュ2つ)送られるのか**、という点がこの仕組みの理解の核心です。IKEの各当事者は、フェーズ1のメッセージの中に次の2種類のNAT-Dペイロードを含めて送ります。

1. **相手(宛先)のIPアドレス・ポートを材料にしたハッシュ**: 自分が接続しようとしている「相手のIPアドレス・ポート」の値を使って計算したハッシュです。
2. **自分(送信元)のIPアドレス・ポートを材料にしたハッシュ**: 自分自身が認識しているローカルのIPアドレス・ポートの値を使って計算したハッシュです。

これを受け取った側は、次の2種類の照合をそれぞれ独立に行います。

| 照合対象 | 比較する値 | 不一致だった場合に分かること |
|---|---|---|
| 届いた「1. 相手(宛先)向け」のハッシュ | 自分が実際に持っているローカルのIPアドレス・ポートから計算したハッシュ | 相手からは自分のアドレスが本来と違う形で見えている、つまり**自分自身がNATの内側にいる** |
| 届いた「2. 送信元向け」のハッシュ | 実際に届いたパケットの送信元IPアドレス・ポート(ネットワーク越しに観測した生の値)から計算したハッシュ | 相手が送った時の値と実際に届いた値が違う、つまり**相手側がNATの内側にいる** |

この2方向の照合をIKEフェーズ1の当事者双方がそれぞれ独立に行うことで、「自分がNAT配下にいるか」「相手がNAT配下にいるか」を、一方だけでなく双方向で正確に判定できます。どちらか一方でも不一致が検出されれば、双方が合意して前述のUDPポート4500へのフロートに進みます。逆にどちらの照合も一致すれば、経路上にNATは存在しないと判断され、UDPポート500のままIKE・ESPのやり取りが継続します。

**注意したいのは、NAT-Dが検出しているのはあくまで「経路上でアドレス・ポートが書き換えられたという事実」であり、それが正規のNAT機器によるものか、悪意ある第三者による改ざん・なりすましかまでは区別できない**という点です。NAT-Dはあくまで「この後UDP4500にフロートすべきかどうか」を判断するための機能的な仕組みであり、セキュリティ上の防御機構ではありません。実際になりすまし・改ざんを防いでいるのは、Cookieを含めたハッシュ自体の予測困難性に加え、この後のIKEフェーズ1で行われる事前共有鍵(PSK)または証明書によるハンドシェイクの認証と、ESPが持つICV(Integrity Check Value)によるパケットごとの完全性検証です。
