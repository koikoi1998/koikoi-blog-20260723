...
- **LDAP(Lightweight Directory Access Protocol)**: ディレクトリサービスに対して検索・追加・変更・削除を行うための標準プロトコルです。AD DSはこのLDAPで問い合わせを受け付けます。LDAPそのものの中身(DN・検索フィルター・ポートの使い分けなど)は[LDAPプロトコルの仕組み](/articles/ad-ldap-protocol-guide)で深掘りします。
...
<details>
<summary>「親子間の信頼関係」とは具体的に何を指すのか、ツリー構造にするメリットは何か</summary>

「信頼関係」というと抽象的に聞こえますが、具体的には「**片方のドメインで認証されたユーザーが、Kerberosの仕組みを使って、もう片方のドメインのリソースにも(適切な権限が付与されていれば)アクセスできる**」という状態を指します。`example.com`と`child.example.com`が親子関係にある場合、`child.example.com`のユーザーアカウントが、`example.com`側のファイルサーバーや共有フォルダーへアクセスすることが、**ドメイン間の信頼関係を個別に設定しなくても、最初から自動的に可能**になっています。この信頼は**双方向**(親→子、子→親のどちらの向きにも)かつ**推移的**(子の子、つまり孫ドメインとも自動的に信頼関係が成立する)という性質を持ちます。

ツリー構造(親ドメイン・子ドメインという階層)を選ぶ実務上のメリットは、主に次の2つです。

- **管理の分散と、名前空間の一貫性の両立**: 各子ドメインごとに、その拠点・部門専用の管理者(ドメイン管理者)を置いて権限を分離しつつ、`tokyo.corp.example.com`のように**組織全体で一貫した命名規則**を保てます。
- **自動的な信頼関係による運用の単純化**: 前述の通り、子ドメインを追加した時点で、フォレスト内の既存ドメインとの信頼関係が自動的に確立されるため、複数の独立したドメイン間でトラストを個別に手動構成する手間がかかりません。

一方で、単に「部署ごとにポリシーを分けたい」程度の要件であれば、ドメインを分割せずOU(組織単位)とグループポリシーの組み合わせで十分なケースがほとんどで、ツリー構造をわざわざ選ぶ主な動機は、**拠点・買収した組織など、DNS名前空間やIT管理の主体そのものを分けたい、法的・組織的な実体の違いがある場合**です。実際に子ドメインを構築し、この信頼関係が自動的に確立される様子を自分の目で確認したい場合は、[マルチドメイン・マルチツリーのADフォレストを構築するハンズオン](/articles/ad-multidomain-handson-guide)を参照してください。
