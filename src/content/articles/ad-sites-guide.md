...
**最初のDCを既定の構成で昇格させた直後は、`Subnets`コンテナーの中身は空っぽのままです。これは異常ではなく、想定通りの初期状態です。** DCの昇格処理は、`Sites`コンテナーの下に`Default-First-Site-Name`というサイトと、そのDC自身のサーバーオブジェクト・NTDS Settingsオブジェクトを自動的に作成しますが、**IPサブネットをサイトへ紐づける作業だけは、自動化されておらず、管理者が`dssite.msc`から明示的に作成・登録する必要があります。** 単一拠点・単一DCの環境では、この登録を省略してもクライアントは既定で`Default-First-Site-Name`に所属するとみなされるため実害はほとんど出ませんが、複数拠点や複数DCの環境を計画している場合は、この記事の以降で扱う内容を理解したうえで、必ず明示的にサブネットを登録してください。
