...
**ドメイン名前付けマスター**は、フォレストへの新しいドメインの追加・削除(および一部のアプリケーションパーティションの追加・削除)を実行できる、フォレスト内で唯一のDCです。新しいドメインを追加する際、ドメイン名前付けマスターは**そのドメイン名がフォレスト内でグローバルに一意であること**をチェックしたうえで追加を許可します。ここでいう「グローバルに一意」とは、**全世界のインターネット上で一意という意味ではなく、あくまで自分が管理するそのフォレストの中で一意**という意味です。`test.local`のような社内でよく使われがちな名前を複数の異なる組織が(それぞれ別のフォレストで)使っていても、フォレストが別である以上は何の問題もありません。ドメイン名前付けマスターが管理・保証しているのは、あくまで「**自分のフォレストの中に、同じドメイン名を持つドメインが2つ以上できてしまう」事故を防ぐこと**であり、フォレストの外の世界にあるドメイン名までは関知しません(そもそもフォレスト外の他組織のドメイン名を知る手段自体がありません)。
...
<details>
<summary>具体例で理解するインフラストラクチャマスターの役割</summary>

たとえば、`domain-a.example.com`にある「営業部」というグループのメンバーに、別ドメイン`domain-b.example.com`のユーザー`tanaka`が含まれているとします。ある日、`domain-b`側の管理者が`tanaka`というユーザー名を`tanaka2`に変更しました。この変更は`domain-b`のDC上ではすぐに反映されますが、`domain-a`側のDCは`domain-b`のオブジェクトそのものを保持していないため、この変更を自動的には知りません。

このとき`domain-a`のインフラストラクチャマスターが、自分が保持している「他ドメインのオブジェクトへの参照」の一覧を定期的にチェックし、参照先(`tanaka`)がリネームされたことを検知して、`domain-a`側が持つ参照情報(グループメンバーの表示名など)を正しい値(`tanaka2`)に更新する、という役割を担います。この更新をインフラストラクチャマスターが行わないと、`domain-a`の管理者が「営業部」グループのメンバー一覧を見たときに、古い名前`tanaka`のまま表示され続ける、いわゆる**ファントムオブジェクト**が発生します。

**単一ドメインのフォレストでは、この問題自体がそもそも発生しません。** 単一ドメインには「他ドメインのオブジェクトへの参照」という概念自体が存在しないため、インフラストラクチャマスターが本来解決しようとしている問題(他ドメインのリネーム・移動の追跡)が起こりえないからです。単一ドメインのフォレストでインフラストラクチャマスターの配置をあまり意識しなくてよいのは、この理由によります。
