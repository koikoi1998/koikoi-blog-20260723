...
**スキーママスター**は、AD DSのスキーマ(オブジェクトの型定義)に対する変更を実行できる、フォレスト内で唯一のDCです。スキーマはフォレスト全体で共有される情報であり、複数のDCが同時にスキーマへ矛盾する変更(たとえば同じ属性名を異なる定義で追加するなど)を加えてしまうと、フォレスト全体のオブジェクト構造の一貫性が壊れかねません。スキーマの変更自体は、Exchange ServerやSCCMなど一部の製品の導入時に発生する程度で、日常的な運用で頻繁に行われる操作ではありません。

### フォレスト全体で1つ:ドメイン名前付けマスター

**ドメイン名前付けマスター**は、フォレストへの新しいドメインの追加・削除(および一部のアプリケーションパーティションの追加・削除)を実行できる、フォレスト内で唯一のDCです。フォレスト内のドメイン名がグローバルに一意であることを保証するためには、「今どんなドメイン名が既に使われているか」を単一の窓口で管理する必要があり、この役目を担っています。
