...
<details>
<summary>`Services`テストが確認している、DCとして必須の各サービスの役割</summary>

`Services`テストが確認している主要なサービスには、それぞれ次のような役割があります。

| サービス | 役割・発生する通信 |
|---|---|
| **Netlogon** | [Netlogonサービスとセキュアチャネルの仕組み](/articles/ad-netlogon-guide)で扱った、DCロケーター(DNSへのSRVレコード問い合わせ)・セキュアチャネルの確立(NRPC、ポート445番のSMB経由)・パススルー認証を担う中核サービス。 |
| **KDC**(Kerberos Key Distribution Center) | [Kerberos認証の仕組み](/articles/ad-kerberos-guide)で扱った、TGT・サービスチケットの発行を担うサービス。UDP/TCP 88番ポートで通信します。 |
| **DNS Server** | AD統合ゾーンに対するクエリ応答・動的更新の受け付けを担うサービス。UDP/TCP 53番ポートで通信します(DCがDNSサーバーの役割も兼ねている場合)。 |
| **W32Time**(Windows Time) | [Windows ServerでNTPサーバーを構築する際の設定値](/articles/windows-ntp-server-guide)で扱った時刻同期サービス。UDP 123番ポートで通信します。Kerberos認証はクライアントとDCの時刻差が一定範囲内であることを前提にしているため、このサービスの異常はKerberos認証にも波及します。 |

これらは単なる比較表としてではなく、「**クライアントがDCへログオンする一連の流れの中で、どのタイミングでどのサービスが呼び出されるか**」という視点で捉えると実務で活きてきます。[Netlogonサービスとセキュアチャネルの仕組み](/articles/ad-netlogon-guide)で扱った、DC発見からKerberos認証までの一連のシーケンス図もあわせて参照してください。

</details>

<details>
<summary>DCロケーター・`_msdcs`・グローバルカタログの関係</summary>

この3つは、それぞれ役割の異なる別々の概念でありながら、密接に連携して動いています。**DCロケーター**は、[Netlogonサービスとセキュアチャネルの仕組み](/articles/ad-netlogon-guide)で扱った通り、クライアントが「自分が接続すべきDC(あるいはグローバルカタログ)」を検索する**処理そのもの**(Netlogonサービスが行う機能)です。この処理が実際に問い合わせているのが、[DNSゾーンとレコードの読み方](/articles/dns-zones-records-guide)で扱った`_msdcs`ゾーンに格納されているSRVレコード群です。そして**グローバルカタログ**は、フォレスト内の他ドメインの情報も部分的に保持している、DCの拡張的な役割の1つであり、`_msdcs`ゾーンの中には`_ldap._tcp.gc._msdcs.<フォレストルートドメイン>`という、グローバルカタログサーバーを専用に検索するためのSRVレコードが用意されています。**つまりDCロケーターが「検索する動作」、`_msdcs`が「検索先の情報を格納する場所」、グローバルカタログが「検索対象となりうる役割の1つ」という、それぞれ別のレイヤーの概念です。**
