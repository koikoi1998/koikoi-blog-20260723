...
<details>
<summary>DCにDNSの役割を追加するだけで、AD統合ゾーンの同期は自動設定されるのか</summary>

**そのDCが、そのドメインで最初に構築されるDC(あるいは新しいドメインの最初のDC)である場合、DNSの役割を追加してゾーンを作成する際に「Active Directory統合ゾーンとして保存する」を選択するだけで、追加の作業なしにAD DSレプリケーションへ自動的に乗ります。** 2台目以降のDCにDNSの役割を追加する場合も、既存のAD統合ゾーンへ参加するだけなので、基本的には役割の追加とゾーンの複製待ちだけで完了します。

一方、**DNSサーバーをDCとは別の(ドメイン参加はしているがDCではない)サーバーに立てる場合は、事情が変わります。** この場合、そのサーバーはそもそもAD DSのレプリケーション機構そのものに参加していないため、ゾーンをAD統合ゾーンにすることはできません(AD統合ゾーンという保存形式自体が、AD DSのデータベースにゾーンデータを格納する方式だからです)。この構成では、通常のゾーンファイル方式を使い、**ゾーン転送**(プライマリ・セカンダリ方式でのゾーンデータのコピー)を明示的に設定するか、あるいはそのサーバーを条件付きフォワーダーの転送先として位置づけるといった、AD DSのレプリケーションに頼らない別の複製・連携手段を自分で構築する必要があります。
...
<details>
<summary>フォワーダー(ISPなど)への通信は許可され、ルートサーバーへの直接の問い合わせは拒否されるのはなぜか</summary>

どちらも「外部との通信」という点は同じなのに、片方は許可され片方は拒否されるのは矛盾しているように見えるかもしれません。ここでの本質は「社内か社外か」ではなく、「**ファイアウォールの許可ルールが、宛先をどれだけ狭く限定できるか**」です。

自力でルートサーバーから再帰的に名前解決を行う場合、最終的にどのサーバーへ問い合わせが飛ぶかは、**解決しようとしている名前によってそのつど変わり、事前に宛先を列挙すること自体が事実上不可能です**(世界中に存在する無数の権威サーバーのどれと通信するか、静的なルールでは表現しきれません)。これをファイアウォールで許可しようとすると、「ポート53番の通信は、宛先を問わずすべて許可する」という、極めて広く、監査の難しいルールにならざるを得ません。このような広すぎるルールは、DNSトンネリングのような情報持ち出しの経路として悪用されるリスクもあります。

一方フォワーダーへの転送は、**あらかじめ決められた、少数の特定のIPアドレスへの通信だけを許可すればよい**という点が決定的に異なります。「ポート53番の通信のうち、この1〜数個のIPアドレス宛だけを許可する」という、宛先を厳密に絞り込んだ狭いルールで実現できます。これは[サーバー構築・運用時の現実的なセキュリティ対策](/articles/practical-server-security-measures-guide)で扱った、**必要な通信先だけに絞ったアウトバウンド制限**という考え方そのものの応用です。フォワーダーを使う構成が好まれるのは、単に「社内から見て許可しやすい」からではなく、**攻撃対象領域(アタックサーフェス)を最小化できる**という、明確なセキュリティ上の理由があるのです。
...
**なぜMicrosoftは、通常のフォワーダーだけをAD DSに格納できない仕様にしているのでしょうか。** これは技術的な制約というより、**両者が解決しようとしている問題の性質が異なる**ことに由来する設計判断だと考えられます。通常のフォワーダーは、「このDNSサーバーが、インターネットへの名前解決をどこに委ねるか」という、**そのサーバー自身が置かれたネットワーク環境(どの拠点の、どの回線を使っているか)に強く紐づく設定**です。複数拠点にDCを分散配置する環境では、拠点ごとに契約しているISPが異なることも珍しくなく、**すべてのDCへ同じフォワーダーが強制的に複製されてしまうと、むしろ拠点ごとに最適なフォワーダーを使い分けられなくなり不都合**です。一方条件付きフォワーダーは、「特定の(パートナー企業や別ドメインの)名前空間をどう解決するか」という、**組織間の信頼関係に近い、フォレスト・ドメイン全体で統一されているべき情報**です。この性質の違いが、複製の要・不要という仕様の違いに反映されている、と理解すると筋が通ります。
