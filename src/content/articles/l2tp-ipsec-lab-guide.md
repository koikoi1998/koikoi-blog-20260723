...
<details>
<summary>FAQ:このハンズオンを始める前に何を準備すればいいですか?</summary>

Proxmox VEでのVM作成、OSインストール、`sudo apt update`・`sudo su -`・`nano`の基本操作に不慣れな場合は、先に準備マニュアルを読んでおくとスムーズです。[ハンズオン準備マニュアル](/articles/handson-prep-guide)を参照してください。
...
<details>
<summary>FAQ:Proxmox VEでVMがなかなか起動しない・動作が重いときはどうすればいいですか?</summary>

多くの場合、ホストCPUの仮想化支援機能(Intel VT-x/AMD-V)がBIOS/UEFIで無効化されており、QEMUがソフトウェアエミュレーション(低速)にフォールバックしていることが原因です。BIOS/UEFI設定で該当機能が有効になっているか確認してください。KVM/QEMUの仕組み自体は[KVM/QEMUの仕組み](/articles/proxmox-internals-guide)で詳しく扱っています。
...
- **PVE内部にもう1段NATを挟む構成**: クライアントVMをNATモードのネットワークに接続すると、クライアント→(NAT)→サーバーという経路になり、意図的にNAT環境を作り出せます。ここでは触れず、検証3で実際の構築手順とあわせて説明します。
...
`chmod 600`は「所有者(root)のみ読み書き可、それ以外のユーザーは読み取りすら不可」にするパーミッション設定です。このファイルには事前共有鍵が平文で書かれているため、もしパーミッションが緩いままだと、同じサーバー上の他の一般ユーザーアカウントから鍵を読み取られてしまいます。実際strongSwanは起動時にこのファイルのパーミッションが緩すぎると警告を出します。パーミッション表記の意味そのものを詳しく知りたい場合は[Linuxのパーミッション(chmod)の仕組み](/articles/linux-file-permissions-guide)を参照してください。
...
`ip range`が、クライアントに払い出す仮想IPアドレスの範囲、`local ip`がサーバー側の仮想ゲートウェイアドレスです。この`local ip`こそ、Step 4でWindowsクライアントの`route print`を見るときにゲートウェイとして表示される値です。`pppoptfile`は、L2TPセッション確立後にxl2tpdが子プロセスとして起動する`pppd`に、どの設定ファイルを読み込ませるかを指定する項目です。ここで指定している`/etc/ppp/options.xl2tpd`はこの後すぐに作成します。つまり`xl2tpd.conf`側はL2TPトンネル/セッションの確立までを担当し、その先のPPPネゴシエーション(認証・IPアドレス払い出し)は`pppoptfile`で指定された設定を使って起動される`pppd`に委譲する、という役割分担になっています。

`xl2tpd.conf`はデフォルトで大量のサンプル行が`;`(セミコロン、コメント記号)付きでコメントアウトされた状態になっています。**上記のように新規で追記する場合は問題ありませんが、もし既存のサンプル行(`; [lns default]`など)を元にコピー&編集する場合は、行頭の`;`を消し忘れないよう注意してください。** `;`が残ったままだとセクション全体が無効なコメント行として読み飛ばされ、`xl2tpd`がこのセクションの存在自体に気づけません。

<details>
<summary>`[lns default]`には`=`による値の設定がないのに、なぜこれで有効化されるのか</summary>

`xl2tpd.conf`のようなINI形式の設定ファイルには、`key = value`という**設定項目の行**と、`[セクション名]`という**セクションの見出し行**の2種類があります。`[lns default]`は後者で、「ここから`default`という名前のLNSセクションが始まる」という宣言そのものが情報であり、`=`で何かの値を設定しているわけではありません。`xl2tpd`の設定パーサーは、コメントアウトされていない`[lns <名前>]`(あるいは`[lac <名前>]`)セクションを見つけると、それだけで「このLNS(LAC)定義は有効である」と認識し、内部に登録します(`enable = yes`のような別のフラグを立てる仕組みは存在しません)。つまり、このセクションが「存在し、かつコメントアウトされていない」こと自体が、有効/無効を切り替えるスイッチの役割を果たしています。後述する検証1でクライアント側に追記する`[lac ...]`セクションも、まったく同じ考え方です。
...
`mtu`/`mru`を1410程度に抑えているのは、L2TP/PPP/ESPなど複数のヘッダーが重なることで実効MTUが縮小するためです。
...
`require-mschap-v2`で認証方式をMS-CHAPv2に固定しています。ユーザー名・パスワードを`/etc/ppp/chap-secrets`に設定します。
...
`/etc/sysctl.conf`に次を追記し、適用します。`sysctl`とカーネルパラメータの反映の仕組み自体は[sysctlとprocfsの仕組み](/articles/linux-sysctl-guide)で詳しく扱っています。
...
<details>
<summary>FAQ:net.ipv4.ip_forward = 1を/etc/sysctl.confに書いたのに反映されません</summary>

`/etc/sysctl.conf`はカーネルが直接読むファイルではなく、`sudo sysctl -p`を実行するか再起動して初めて`/proc/sys`へ反映されます。ファイルを保存しただけでは稼働中のカーネルには反映されません。

</details>

ファイアウォール([iptables](/articles/linux-iptables-guide))では、IKE・NAT-T・ESP・L2TPに必要なポート/プロトコルを許可し、クライアントの仮想IPアドレス帯がインターネットに出られるようMASQUERADEを設定します(`eth0`は実際のWAN側インターフェース名に置き換えてください)。インターフェース名がわからない場合は、次のコマンドでデフォルトルート(≒WAN側)に使われているインターフェースを確認できます。
...
<details>
<summary>FAQ:iptablesで許可したルールは、サーバーを再起動しても保持されますか?</summary>

いいえ、netfilterへ登録されたルールはカーネルのメモリ上の状態であり、デフォルトでは再起動すると消えます。`iptables-persistent`パッケージや`iptables-save`/`iptables-restore`で明示的に永続化する必要があります。
...
`;`の消し忘れに関する注意や、セクションの見出し行が`=`を伴わずに有効化される仕組みは、サーバー構築時(Step 3)の`[lns default]`で説明した通りです。エラー調査の基本的な考え方や`journalctl`の使い方は[journalctlでエラーログを調査する](/articles/linux-journalctl-guide)で詳しく扱っています。
...
```
# client        server  secret                    IP addresses
labuser         *       "サーバー側と同じパスワード"      *
```

サーバー側の`chap-secrets`と同じ理由で、`server`フィールドは`*`にしています。クライアントのpppdは、CHAPチャレンジを送ってきた相手(サーバー)が名乗った名前をこの`server`フィールドと突き合わせますが、その名乗る値は`options.xl2tpd`に`name`オプションを明示していない今回の構成ではサーバーのホスト名など環境依存の値になり、`xl2tpd.conf`の`[lns default]`に設定した`L2TPLab`(これはL2TPプロトコル上でLNSが名乗る名前であり、pppdの認証用の名前とは別物です)とは一致しません。ここを固定値にしてしまうと、環境によって認証がログ上はっきりしたエラーが出ないまま失敗しうるため、`*`にしておきます。
...
キャプチャを開始した状態で、上で使った`ipsec up L2TP-PSK`→`xl2tpd-control connect-lac L2TPLab`を再度実行して接続し、接続完了後に`Ctrl+C`でキャプチャを停止します。取得した`l2tp-capture.pcap`を[Wireshark](/articles/wireshark-guide)で開くと、次のような流れが実際のパケットとして観測できます。
...
<details>
<summary>FAQ:IKEフェーズ1(6往復)・フェーズ2(3往復)という回数は必ず固定ですか?</summary>

理論上の基本的な回数です。実装やリトライ、ネットワーク状況によって前後することがあります。上の手順は、実際のキャプチャで理論値通りになっているかを自分の環境で確認するためのものです。
...
Windows標準のVPN設定画面(設定→ネットワークとインターネット→VPN→VPN接続を追加)から、種類に「L2TP/IPsec(事前共有キー)」を選び、サーバーアドレス・ユーザー名・パスワード・事前共有鍵を入力して接続します。

<details>
<summary>FAQ:接続すると「リモート コンピューターへの接続を確立できなかったので、この接続に使われたポートは閉じています」と表示されます</summary>

サーバー側のpppdログに`couldn't find any suitable secret (password)`が出ている場合、原因は`/etc/ppp/chap-secrets`に該当ユーザーの認証情報が見つからないことです。本記事の`chap-secrets`はサーバー名フィールドを`*`にしているため、クライアント名(ユーザー名)とパスワードさえ正しいエントリを追加すれば、Linuxクライアントと同じファイルでWindowsクライアントも認証できます。Windowsの接続設定画面で入力したユーザー名・パスワードと、`chap-secrets`のエントリが一致しているか確認してください。

</details>

接続後、コマンドプロンプトで次を実行します。
...
### サーバー自身がゲートウェイとして機能していることを`tracert`で確認する

ここまでの`route print`だけを見ると、「ゲートウェイが数値で表示されない=`10.10.10.1`はゲートウェイとして機能していない」かのような印象を受けるかもしれません。しかし実際には`10.10.10.1`(サーバーの仮想IP)は、それ以外の宛先向けパケットを中継する、紛れもなくこのリンクの唯一の次ホップ(=ゲートウェイ)として機能しています。これを実際の経路として確認するには、VPN接続中に外部の宛先へ`tracert`を実行します。

```
tracert 8.8.8.8
```

出力の最初のホップ(1番目)が`10.10.10.1`(サーバーの`local ip`)になっていることを確認してください。`route print`の表示上は「リンク上」としか見えなかった相手が、実際のパケットの通り道としては確かにサーバー自身であり、サーバーがVPNクライアントにとってのゲートウェイとして機能していることが、経路情報として裏付けられます。「ゲートウェイの値が数値として表示されない」ことと「ゲートウェイが存在しない・機能していない」ことはまったく別の話である、という点をこの結果は示しています。

## 検証3: NAT-Tを意図的に発生させて検証する

ここまでの検証1・検証2は、クライアントVMをブリッジ(`vmbr0`)に直結し、サーバーVMと同じLANセグメントに置いた状態で行ってきました。この構成では、クライアント→サーバー間にNAT機器が挟まらないため、NAT-Tは発生しません。NAT-Tの動作を意図的に発生させて観測するには、クライアント→(NAT)→サーバーという経路を作る必要があります。Proxmox VEには、VirtualBoxのような「NATモード」のネットワークがあらかじめ用意されているわけではないため、まずはこの経路自体をPVE上に構築します。

### PVE内部にもう1段NATを挟む環境を構築する

ここでは、Step 4でサーバー用VMに対して行ったのと**まったく同じ技術(`ip_forward`+`iptables MASQUERADE`)を、PVEホスト自身に対して適用する**ことで、簡易的なNATネットワークを作ります。
...
これで、クライアントVM→(PVEホストがNAT)→サーバーVMという経路の土台が完成します。Step 4でサーバー用VMに設定したのと同じ`ip_forward`・`MASQUERADE`という2つの仕組みを、今度はPVEホスト自身に適用しているだけだと分かると、NATの正体が「特別な機能」ではなく「経路の途中にあるどのLinuxホストにも仕込める、ごく普通の仕組み」であることが実感できるはずです。

### クライアントを新しいNATネットワークへ移動する

検証2までは、クライアントVMを`vmbr0`に接続した状態で手順を進めてきました。ここからは、そのクライアントVMを先ほど作った`vmbr1`側へ移動し、実際にNAT越しの接続を作ります。サーバーVM側は`vmbr0`に接続したまま、設定変更も不要です。

1. **クライアントVMのネットワーク接続先を変更する**: PVEの管理画面でクライアントVMを選び、「Hardware」→ネットワークデバイスの接続先ブリッジを`vmbr0`から`vmbr1`に変更します。
2. **クライアントVM側でIPアドレスを設定し直す**: `vmbr0`ではDHCPで家庭内LANのアドレスを取得していたはずですが、`vmbr1`には自動でアドレスを配る機能がないため、固定IPアドレス(例: `192.168.100.10/24`、ゲートウェイ`192.168.100.1`)を手動で設定します。
3. **サーバーへの疎通を確認する**: VPN接続を試みる前に、まずNAT経由でサーバーVMの実アドレスに`ping`が通ることを確認してください。ここで疎通が取れない場合、後続のIKEネゴシエーションも当然失敗するため、先にネットワークレイヤーの問題を切り分けておく意味があります。

   ```bash
   ping -c 3 <サーバーのIPアドレス>
   ```

なお、クライアント側の`/etc/ipsec.conf`の`right=<サーバーのIPアドレス>`は、検証1で設定した値のままで変更不要です。NATが変換するのはクライアントの**送信元**アドレスであり、接続先であるサーバーのアドレス自体はNATの内側・外側どちらから見ても変わらないためです。疎通が確認できたら、検証1と同じ手順(`sudo ipsec up L2TP-PSK`→`sudo xl2tpd-control connect-lac L2TPLab`)で接続してください。なお、検証1・検証2の手順に戻りたくなった場合は、クライアントVMの接続先ブリッジを`vmbr0`に戻し、IPアドレス設定も元に戻せば再び同じ検証を行えます。

### NAT-Tの発生を`tcpdump`で観測する

クライアントがNAT越しになった状態で、サーバー側で次のキャプチャを実行しながら接続してみてください。

```bash
sudo tcpdump -i any 'udp port 500 or udp port 4500' -w /tmp/nat-t-capture.pcap
```

このキャプチャを[Wireshark](/articles/wireshark-guide)で確認すると、IKEフェーズ1のメッセージの中にNAT-D(NAT Detection)ペイロードが含まれていること、その後の通信がUDPポート500から<strong>UDPポート4500へフロート(切り替わる)</strong>こと、そしてESPパケットがUDPヘッダーでさらにカプセル化されて送られてくることが、実際のパケットとして確認できます。あわせて、Windowsクライアントを使っている場合は、サーバー自身がNATの内側にある(二重NAT)構成でクライアント側の接続が失敗し、レジストリの`AssumeUDPEncapsulationContextOnSendRule`を`2`に設定することで解決する、という挙動も再現・検証できます。
...
このハンズオンで実際によく発生する事象を、「まずどのコマンドで確認するか」を軸に一覧にしています。より詳しい原因の解説は、[journalctlでエラーログを調査する](/articles/linux-journalctl-guide)も参照してください。
...
4. **`ipsec`/`xl2tpd`のSAは確立しているのに`ppp0`が作成されない**: `sudo journalctl -t pppd`で確認できるオプションファイル(`options.xl2tpd`/`options.l2tpd.client`)のタイポや、環境によっては使えないオプションが原因であることが多いです。
...
<details>
<summary>FAQ:エラーが起きたとき、strongSwan(ipsec)・xl2tpd(l2tp)・pppdのどのログをどの順番で見ればいいですか?</summary>

接続はIKE(ipsec)→L2TPトンネル/セッション(xl2tpd)→PPP認証(pppd)の順に確立するので、切り分けもこの順番で進めるのが効率的です。まず`sudo ipsec statusall`でIKE/IPsec SAが上がっているかを確認し、上がっていなければ`journalctl -u strongswan-starter`を見ます。SAは上がっているのに`ppp0`が出てこない場合は`journalctl -u xl2tpd`、それでも原因が分からなければ`journalctl -t pppd`(上の検証1で触れた通り、pppdはsystemdユニットではなくsyslogの識別子でログを出すため`-t`で絞り込みます)を確認します。サーバー側・クライアント側どちらも同じ考え方で追えるので、両方のホストで同じ時刻帯のログを突き合わせると原因を特定しやすくなります。

</details>

<details>
<summary>FAQ:ipsec upでNO_PROPOSAL_CHOSENが出て、ike=の値からスペースを消してクライアントを再起動しても直らないのはなぜですか?</summary>

strongSwanの暗号スイート一覧(`ike=`/`esp=`)はカンマ区切りで厳密にパースされるため、カンマの後に空白が入っていると「不正なアルゴリズム名」として扱われ、その提案(プロポーザル)全体が拒否され`NO_PROPOSAL_CHOSEN`になります。厄介なのは、設定ファイルを直接編集しても稼働中のcharonデーモンには即座に反映されない点と、失敗したネゴシエーションの状態がデーモン内に残ってしまう場合がある点です。クライアント側だけ設定を直して再起動しても、サーバー側が古い状態(あるいは以前の失敗したネゴシエーションの痕跡)を持ったままだと再度失敗することがあり、結局サーバー側も含めて両方を再起動して初めて、双方が新しい設定でクリーンな状態から鍵交換をやり直す形になり解消します。設定ファイルを編集したら、片側だけでなく両側を`sudo ipsec restart`(または`systemctl restart strongswan-starter`)しておくと、この種の問題を避けやすくなります。
