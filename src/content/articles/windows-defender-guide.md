...
**オンデマンドスキャン**(クイックスキャン・フルスキャン)は、これとは別に、**「今まさにディスク上に存在しているファイルすべて、あるいは一部を、まとめて検査する」**という、任意のタイミングで実行する検査です。日常的な脅威の大半はリアルタイム保護によって既に防がれているという前提の上で、**オンデマンドスキャンは「見落としがないかを確認する、補完的な検査」**という位置づけになります。
...
また、**Attack Surface Reduction(ASR)ルール**は、マルウェアがよく悪用する特定の挙動パターン(たとえば、Office文書からの子プロセス起動や、スクリプトによる実行可能ファイルのダウンロードなど)自体を、企業のポリシーとしてあらかじめ禁止しておく機能です。**検知ではなく「そもそも悪用されうる経路を塞ぐ」**という、より予防的なアプローチであり、企業向けの管理機能(Microsoft Defender for Endpointなど)の中で設定されることが一般的です。
