...
- **この記事で得られること**: Windows ServerでNTP(Network Time Protocol)サーバー機能を構成する際に必ず登場する<strong>Stratum(階層)</strong>という考え方、**AnnounceFlags**というレジストリ値が何を制御しているのか、そして**なぜフォレストルートのPDCエミュレータだけが、外部の時刻源への明示的な同期設定を必要とするのか**を体系的に理解します。
...
NTPは、<strong>Stratum(階層)</strong>という番号によって、時刻源としての信頼度・階層構造を表現します。
...
<strong>既定値は`10`(16進数で`0x0A`、つまり`0x02`+`0x08`の組み合わせ)</strong>です。この既定値の意味は、<strong>「自分がフォレストルートのPDCエミュレータであれば自動的に時刻サーバー・信頼できる時刻源として振る舞い、そうでなければ振る舞わない」</strong>という、条件付きの動作です。この既定値のおかげで、管理者が個々のDCに対して手動で設定を切り替える必要はなく、**FSMO転送によってPDCエミュレータの役割が別のDCへ移った場合でも、AnnounceFlagsの設定自体はそのままで、新しいPDCエミュレータが自動的にこの役割を引き継ぎます。**
...
### 「時刻サーバーとして動作する」と「信頼できる時刻源として動作する」は何が違うのか

AnnounceFlagsの4つのビットを見ると、`0x01`/`0x02`(**時刻サーバー**)と`0x04`/`0x08`(**信頼できる時刻源**)という、似た言葉が並んでいて混同しやすいところです。この2つは、**同じ動作の2段階のスイッチ**だと理解すると整理できます。

- **「時刻サーバーとして動作する」(`0x01`/`0x02`)**: そのマシンが、UDP 123番ポートへのNTP問い合わせに**応答するかどうか**を制御します。このビットが立っていれば、他のマシンから明示的に(`NtpServer`に指定するなどして)問い合わせれば時刻を返しますが、それだけでは「階層構造の中で自動的に選ばれる上位の時刻源」にはなりません。
- **「信頼できる時刻源として動作する」(`0x04`/`0x08`)**: そのマシンの時刻を、**NT5DS方式で自動的に上位を探しているクライアントが、同期先として正式に採用してよいかどうか**を制御する、もう一段上のフラグです。

**あるマシンが「信頼できる時刻源」を名乗るには、その前提として「時刻サーバー」でもある必要があります**(信頼できるのに問い合わせに応答しない、という状態は成立しません)。既定値`0x0A`が`0x02`(時刻サーバー、自動)と`0x08`(信頼できる時刻源、自動)の組み合わせになっているのはこのためで、フォレストルートのPDCエミュレータは「問い合わせに応答する存在」であると同時に「自動選択の対象として信頼してよい存在」の両方を兼ねる必要があるということです。

### PDCエミュレータのNTP機能は、DCへの昇格時にインストールされる機能と別物なのか

結論として、**別物ではありません**。W32Time(Windows Timeサービス)は、DCに昇格しているかどうかに関わらず、**すべてのWindowsマシンに標準で搭載されている単一のサービス**です。DCへ昇格する際に「NTPサーバーという新しい機能が追加インストールされる」わけではなく、**既存のW32Timeサービスの設定(`Type`やAnnounceFlagsの自動ビット)が、AD DSの階層構造に合わせて自動的に切り替わる**というのが実態です。

- 一般のドメイン参加済みマシン: `Type=NT5DS`で、AD DSの階層構造に従って自動的に上位のDCを頼るだけの「クライアント」的な設定になります。
- DC(フォレストルートのPDCエミュレータ以外): 同じくNT5DSですが、AnnounceFlagsの自動ビットにより、配下のメンバーサーバー・クライアントPCに対しては「時刻サーバー」として振る舞います。
- フォレストルートのPDCエミュレータ: 前述の通りドメイン内に頼れる上位がいないため、`Type=NTP`など外部の時刻源への明示的な同期設定が必要になり、AnnounceFlagsの自動ビットにより自身が「信頼できる時刻源」として振る舞います。

つまり、**PDCエミュレータのNTP機能とDC昇格時のNTP機能は、名前が違う2つの機能ではなく、同じW32Timeサービスが置かれた立場に応じて異なる設定値で動作しているだけ**です。「PDCエミュレータだけ特別なソフトウェアが入っている」という誤解は、実務でのトラブルシューティングの妨げになりやすいポイントなので、注意してください。
...
- 「時刻サーバー」(問い合わせに応答するか)と「信頼できる時刻源」(自動選択の対象として採用してよいか)は2段階のスイッチであり、信頼できる時刻源であるためには時刻サーバーでもある必要があります。
- PDCエミュレータのNTP機能は、DC昇格時に別途インストールされる機能ではなく、すべてのマシンに標準搭載されているW32Timeサービスが、置かれた立場に応じて異なる設定値で動作しているだけです。
