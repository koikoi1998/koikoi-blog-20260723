...
### 「1回の認証で複数のサービスにアクセスできる」の正確な意味

上のシーケンス図では②③の手順を1つのサービスに対してだけ示していますが、実務でよく言われる「一度ログインすれば、複数のサービスに個別に認証し直さなくてよい」というKerberosの利便性は、正確には「**TGTを1回取得すれば、その後は"サービスごとに"TGSへ問い合わせて個別のサービスチケットを取得するだけで済み、AS-REQによるユーザー本人確認(パスワードを使った事前認証)をやり直す必要がない**」という意味です。つまり、1枚のチケットがすべてのサービスに万能で使えるわけではなく、**アクセスするサービスの数だけ、②③の手順(TGS-REQ/TGS-REP、AP-REQ)がその都度発生します。** ただし、この②③の手順にはユーザーのパスワードそのものは一切関与せず、手元のTGTを提示するだけで完結するため、ユーザー体験としては「一度ログインすれば、あとは意識せずに複数のサービスへアクセスできる」ように感じられる、というわけです。

### Kerberosのやり取りは、通信のどのタイミングで発生しているのか

ここまでの説明が「KDCと通信する」という抽象的な話に感じられる場合、実際のネットワーク通信の中でこれがどのタイミングで挟まっているのかを具体的にイメージすると理解が進みます。たとえばクライアントが`\\dbserver\share`のような共有フォルダーへアクセスしようとする場合、大まかには次の順序で通信が発生します。

1. **名前解決**: `dbserver`というホスト名を、DNSへの問い合わせでIPアドレスへ変換します。
2. **TCP接続の確立**: 得られたIPアドレスへ向けて、目的のポート番号(SMBなら445番)でTCP接続を確立します。
3. **アプリケーション層のプロトコル開始とKerberos認証**: TCP接続が確立した後、SMBなどのアプリケーション層のプロトコルのやり取りが始まりますが、**この中で、クライアントが裏側で(まだTGTを持っていなければ)KDCとの間でAS-REQ/AS-REPを、目的のSPNに対して(まだサービスチケットを持っていなければ)TGS-REQ/TGS-REPを行い、取得したサービスチケットをAP-REQとして相手サーバーへ提示する**、という一連のやり取りが挟まります。TGT・サービスチケットは一定期間キャッシュされるため、毎回すべての手順が発生するわけではありません。

**Kerberos認証は、IPルーティングやDNS解決とは別の階層(アプリケーション層に近い、認証専用のやり取り)として、TCP接続確立の"後"に行われる**、という位置づけを押さえておくと、pingやtracertのようなネットワーク層の疎通確認だけではKerberosの問題を検出できない理由が見えてきます。KDCとの通信自体はUDP/TCP 88番ポートで行われます。
...
### よくある勘違い:SPNは「アクセスする側」ではなく「サービスを提供する側」に登録される

ここは非常に間違えやすいポイントです。「特定のサービスにアクセスできるアカウントには、そのアカウントの属性に対象サービスのSPNが登録される」という理解は、**登録される向きが逆**です。SPNは、「そのサービスへアクセスしてくる、あらゆるクライアント」ではなく、「**そのサービス自身を実行しているアカウント、ただ1つ**」に登録されます。

たとえば`dbserver`というサーバー上でSQL Serverのサービスが動いている場合、`MSSQLSvc/dbserver.corp.example.com:1433`というSPNは、**SQL Serverのサービスを実行しているアカウント(サービスアカウント)ただ1つ**に登録されます。このSQL Serverに接続してくるクライアント(利用者)のアカウントが何百・何千あっても、**それらのアカウントの側には、このSPNは一切登録されません。** クライアント側は、あくまで「このSPN宛てにアクセスしたい」という要求をKDCへ提示するだけの立場であり、SPNを自分のアカウントの属性として持つ必要はまったくありません。

**「クライアントが、アクセスを許可されているサービスの一覧をSPNとして持っている」のではなく、「サービスの側が、"私はこのSPNで呼び出される者です"という自己紹介をSPNとして持っている」**、という向きで理解すると正確です。

### 前提条件:クライアント・サーバーの双方がドメイン参加している必要がある

ここまでの説明はすべて、**クライアント・サービスを提供するサーバーの両方が、同じフォレスト内でドメインに参加している(あるいは信頼関係のある別ドメインに参加している)ことを前提**にしています。SPNもservicePrincipalName属性も、AD DS上のアカウントオブジェクトに対して登録される情報であるため、そもそもAD DSにアカウントが存在しない(ドメインに参加していない)コンピューターやユーザーは、Kerberos認証の当事者にはなれません。ドメインに参加していない端末やサービスへのアクセスでは、Kerberosではなく別の認証方式(ローカルアカウントによる認証など)が使われます。

### 「security context(セキュリティコンテキスト)」とは何か

本記事や関連記事で繰り返し登場する**セキュリティコンテキスト**とは、**あるプロセスやサービスが、実行時にどのアカウントの権限・識別情報を背負って動作しているか**を表す言葉です。「このサービスは、コンピューターアカウント自身のセキュリティコンテキストで動いている」といえば、そのサービスが行うすべての操作が、コンピューターアカウント自身の権限で実行されている、という意味になります。専用のサービスアカウントでサービスを実行する場合は、そのサービスのセキュリティコンテキストが、コンピューターアカウントから専用のサービスアカウントへ切り替わる、とイメージすると分かりやすくなります。
