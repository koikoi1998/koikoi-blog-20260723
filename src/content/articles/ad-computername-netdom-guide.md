...
- **NetBIOS名とDNSホスト名**: Windowsのコンピューターは、伝統的に15文字までの短い**NetBIOS名**と、`hostname.example.com`のような**DNSホスト名**(FQDN)という、2種類の名前を持ちます。現在では多くの用途でDNSホスト名が主に使われますが、レガシーな互換性のためNetBIOS名も引き続き併存しています。なぜ15文字までなのか、なぜ2種類の名前が共存しているのかという歴史的経緯は[NetBIOS名とDNSホスト名、なぜ2つの名前が共存しているのか](/articles/ad-netbios-dns-history-guide)で深掘りします。
- **SPN(Service Principal Name)**: Kerberos認証において、「このサービスはこの名前で呼び出されたときに、このアカウントとして応答する」ことを紐づける識別子です。`HOST/コンピューター名`という形式のSPNが、コンピューターアカウントに既定で登録されています。SPNの仕組みそのものの詳しい解説は[SPN(サービスプリンシパル名)の仕組みを『上位1%』の視点で理解する](/articles/ad-spn-guide)で扱います。
- **コンピューターアカウント**: ドメインに参加した端末1台ごとに、AD DS上に作成される専用のアカウント(オブジェクト)です。ユーザーアカウントと同様にパスワード(実際にはコンピューター自身が定期的に自動更新するランダムな値、既定では30日ごと)を持ちます。
- **セキュアチャネル**: コンピューターがこのパスワードを使ってDCに対して自分自身を認証し、確立するNetlogon経由の暗号化された通信路です。「コンピューター自身がAD DSに登録された自分のパスワードを正しく提示できる状態」がセキュアチャネルが健全であることの実体であり、AD DS側の記録(パスワード)とコンピューター自身がローカルに保持する記録が食い違うと、セキュアチャネルは切れます。このセキュアチャネルおよびNetlogonサービスの詳しい仕組みは、[Netlogonサービスとセキュアチャネルの仕組みを『上位1%』の視点で理解する](/articles/ad-netlogon-guide)で深掘りしています。
...
5. 対処として実施された「ホスト名を変更し、一度ワークグループへ戻して、再度ドメイン参加する」という手順は、この診断と正確に整合します。DC①のホスト名を変更することで名前の重複が解消され、ワークグループへ戻すことで壊れかけていた古いドメイン参加状態をいったん完全に破棄し、新しい名前で再度ドメイン参加することで、DC②のコンピューターアカウントとは別の、まったく新しいコンピューターアカウントが作成されます。これにより、**それ以降DC②のコンピューターアカウントに一切手を触れずに済む**ようになり、少なくとも「DC①が繰り返しDC②のパスワードを上書きしてしまう」という進行中の事故は止まります。

<details>
<summary>補足: この操作だけでDC②側の破損は"自動的に"直るのか</summary>

ここで一点、注意すべき誤解があります。DC①をワークグループへ戻して新しい名前で入れ直しても、**それによってDC②のコンピューターアカウントのパスワードが「上書きされる前の値」に巻き戻るわけではありません**。DC①側の操作はあくまで「今後DC②のアカウントに触れなくする」ことが目的であり、過去に発生した上書きそのものを取り消す効果はありません。

「DC①をワークグループへ戻す操作そのものが、AD DS側からDC①(=DC②と同名だった)のアカウント情報を削除し、それによって上書き前の状態に戻ったのではないか」という見立ても一見もっともらしいのですが、これは実際には成立しません。第一に、[AD移行後のクリーンアップを『上位1%』の視点で理解する](/articles/ad-migration-cleanup-guide)で扱った通り、ドメインを脱退してもAD DS上のコンピューターアカウント自体は既定では削除されず残り続けます。第二に、この事例のようにログオン自体が失敗している状況では、DC①をワークグループへ戻す操作はDCへ到達できないままローカルだけで完結する「オフラインでの脱退」になっている可能性が高く、その場合はそもそもAD DS側のデータに一切触れることができません。つまり、ワークグループへ戻す操作自体は、AD DS側のパスワード情報を消したり書き戻したりする効果を持っていない、と考えるのが妥当です。

それでもこの事例でログオンが復旧したのは、AD DSがコンピューターアカウント(信頼アカウント)のパスワードについて、**最新の値だけでなく1世代前の値も一定期間は有効なものとして許容する**という仕組みを持っているためだと考えられます。DC②は上書きされたことに気づかないまま、自分がローカルに保持している(=1世代前になった)パスワードでNetlogonの認証を試み続け、この許容期間内であれば偶然にも認証が通ってしまう、という状態です。これは**あくまで一時的・偶発的な延命**であり、確実な修復ではありません。実務でこの種の事故に遭遇した場合は、様子見で自然回復を期待するのではなく、DC②自身で`Test-ComputerSecureChannel -Repair`(または`netdom resetpwd`)を実行し、DC②のローカルの認証情報とAD DS側の記録を明示的に再同期させることが、確実な復旧手順です。
...
### 結局、`sysdm.cpl`と`netdom computername`のどちらで作業すべきか

手順4の「新DCの名前を、旧DCが持っていた名前へ変更する」を実施する際、`sysdm.cpl`でも`netdom computername`でも、コンピューターアカウントのSPN・DNSレコード・NetBIOS名は最終的にはどちらも正しく更新されます。**単発で、ダウンタイムを許容できる作業であれば、どちらを使っても結果は同じ**です。それでも実務で`netdom computername`が推奨されることが多いのは、次の2点が理由です。

1. **切り替えの瞬間を「重ねられる」**: `sysdm.cpl`は旧名から新名への置き換えを1回の再起動でアトミックに行うため、再起動が完了した瞬間から旧名は一切応答しなくなります。一方`netdom`は`/add`で新名を代替名として先に登録できるため、**旧名で稼働させたまま、新名でも同時に到達可能な期間**を作れます。クライアントのDNSキャッシュや、旧名をハードコードしたアプリケーションの向き先切り替えを、無停止に近い形で段階的に進められるのはこのためです。
2. **リモートから、対話操作なしで実行できる**: `sysdm.cpl`はGUI操作が前提ですが、`netdom computername <対象コンピューター名> /add:...`のように、**対象を明示的に指定してコマンドラインから実行**できます。AD移行のように複数台を計画的に改名する場合、スクリプト化・自動化しやすいことも実務上の利点です。

つまり「どちらで作業しても最終結果(SPN・DNS・NetBIOS名への反映)は同じだが、`netdom`は移行作業に必要な"重ね合わせ"と"自動化"を提供してくれる」という整理になります。
...
1. **特定のドメインコントローラーで急にNetlogon関連のエラーが多発する、セキュアチャネルが切れる**: そのDC自身のコンピューターアカウントのパスワードが、意図せず変更・リセットされていないかを疑います。`Test-ComputerSecureChannel`(該当コンピューター上で実行)で、セキュアチャネルの状態を確認でき、`Test-ComputerSecureChannel -Repair`(または`netdom resetpwd`)で、AD DS側の記録に合わせてローカルの認証情報を明示的に再同期できます。ただし対象がDCの場合、`-Repair`はドメインコントローラーには使えないため、DCについては後続記事で扱う`repadmin`や、必要に応じたDCの再構築で対処します。
