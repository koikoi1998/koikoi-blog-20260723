...
<details>
<summary>冗長化構成の実サーバーIP・代表IP(VIP)をアドレスプールと同じサブネットに共存させる設計</summary>

実務では、RRASサーバー自身が運用系・待機系の2台構成で冗長化されており、かつVPNクライアントがアクセスする社内アプリケーションも同様に冗長化されている、というケースが珍しくありません。この場合、同一の`10.0.20.0/24`セグメント上に、①VPNクライアント用アドレスプール、②運用系サーバーの実IP(例: 10.0.20.2)、③待機系サーバーの実IP(例: 10.0.20.3)、④両者の代表IP=VIP(例: 10.0.20.4)という、目的の異なる複数のIPアドレス群が共存することになります。

ここで、後述する「静的アドレスプールの先頭アドレスがRRASサーバー自身の内部インターフェース用に予約される」という挙動を踏まえずに、単純に`10.0.20.1〜10.0.20.50`という1本の範囲だけをプールとして指定してしまうと、先頭の10.0.20.1こそRRAS自身の内部アドレスとして予約されるものの、10.0.20.2〜10.0.20.4は「除外」されないままプールに含まれてしまい、運用系・待機系サーバーやVIPが使っているアドレスがそのままVPNクライアントに払い出されて、IPアドレスの重複が起こり得ます。

これを避けるには、上で見た「複数レンジに分割して除外する」テクニックをそのまま応用します。たとえば、①10.0.20.1のみを含む1個のIPだけの範囲、②10.0.20.5〜10.0.20.50という範囲、の2つを別々のレンジとしてStaticAddressPoolに登録します(10.0.20.2〜10.0.20.4はどちらのレンジにも含めません)。RRASは最初に登録したレンジの先頭アドレスを内部アドレスとして予約する仕様のため、10.0.20.1を含むレンジを最初に登録しておけば、内部アドレスは意図通り10.0.20.1になり、10.0.20.2〜10.0.20.4はどのレンジにも属さないためVPNクライアントへ払い出されることはなく、実サーバー・VIP用として安全に予約された状態を保てます。VPNクライアントには10.0.20.5以降が払い出されます。

このように、**冗長化構成そのものはRRASのアドレスプール設計と直接の関係はなく、あくまで「プールの範囲に、他の用途で固定的に使われているIPアドレスを含めない」という、既に述べた除外設計の一例**として扱えます。

</details>

<details>
<summary>特定のVPNクライアントに、接続のたびに同じIPアドレスを持たせることはできるか</summary>

ここまで見てきた静的アドレスプールおよびDHCPリレーは、いずれも「接続してきた順に、空いているアドレスを払い出す」という動的な割り当てが基本であり、同じクライアントが再接続しても、前回と同じIPアドレスが払い出される保証はありません。

特定のユーザーにだけ常に同じIPアドレスを固定で持たせたい場合は、これらのプールの仕組みとは別に、**ユーザーアカウント単位での静的IP割り当て**という機能を使います。RRASの認証をActive Directoryのユーザーアカウントで行っている場合、「Active Directoryユーザーとコンピューター」の対象ユーザーのプロパティ→「ダイヤルイン」タブにある **「次のIPアドレスの割り当て」→「静的IPアドレスの割り当て」** の設定でIPアドレスを指定すると、そのユーザーが接続してきたときは、静的アドレスプールやDHCPリレーの払い出し結果によらず、指定したIPアドレスが優先的に割り当てられます(NPSをRADIUSサーバーとして利用している構成では、ネットワークポリシーの「IP設定」に相当する項目で同様の指定が可能です)。

この方式を使う場合、ユーザーごとに固定したIPアドレスは、一般のプール範囲やDHCPスコープの払い出し対象からあらかじめ除外しておく必要があります。除外しておかないと、固定IPを割り当てたユーザーが未接続の間に、その同じアドレスが別のクライアントへ動的に払い出されてしまい、二重割り当てが発生し得ます。
...
なお、この代表IP(VIP)は、あくまで社内LAN側に存在する接続先の1つであり、VPNクライアント自身の払い出しIPアドレス(点対点リンクの「自分側」のアドレス)として使われることはありません。クライアントは、静的アドレスプールなりDHCPリレーなりで通常どおり自分自身のIPアドレス(例: 10.0.20.10)を受け取った上で、その点対点リンク経由で、あて先としてVIP(例: 10.0.20.4)へ通信します——つまりVIPは、前述の「なぜ同一セグメントに見えるのにゲートウェイが必要なのか」で見た社内LANの実サーバー(10.0.20.2)と何ら変わらない、単なるルーティング先の1つに過ぎません。

同様に、冗長化構成を組む個々の実サーバー(運用系・待機系それぞれの実IP)についても、クライアントのアプリケーションが直接そのIPアドレス宛に通信することがなければ、VPNクライアントへ「払い出す」必要は一切ありません。必要なのは、これらのアドレスをアドレスプールの払い出し対象から確実に除外しておくこと(前述の複数レンジ分割によるテクニック)だけであり、それさえ満たしていれば、RRASのルーティングによって社内LAN側のどのアドレスにも到達可能な状態は変わらないため、特別な設定を追加する必要はありません。
