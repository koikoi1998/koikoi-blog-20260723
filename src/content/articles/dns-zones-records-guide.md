...
<details>
<summary>`_msdcs`ゾーンは「フォレストルートドメインのDC」だけを検索するためのものなのか</summary>

ここは誤解しやすいポイントです。`_msdcs.<フォレストルートドメイン名>`というゾーン名だけを見ると、「フォレストルートドメインのDCを検索するための特殊ゾーン」のように読めてしまいますが、正確には**このゾーン名の中の「フォレストルートドメイン名」の部分は、あくまでゾーン自体の置き場所(namespaceの起点)を表しているだけ**です。実際にこのゾーンの中には、フォレストルートドメインだけでなく、**フォレスト内のすべての子ドメインについて、それぞれの「そのドメイン自身のDCを検索するためのレコード」が、ドメイン名ごとに区別された形でまとめて格納されています**(たとえば`_ldap._tcp.dc._msdcs.corp.example.com`は、`corp.example.com`が子ドメインであっても、この1つの`_msdcs`ゾーンの中に存在します)。つまり「ドメインごとに別々の`_msdcs`ゾーンを用意する」のではなく、「**フォレスト全体で1つの`_msdcs`ゾーンに、全ドメイン分のレコードをまとめて収容する**」という設計です。

ではなぜ、各ドメインが自分の通常の前方参照ゾーンの中に(フォレストルートドメインのDCのレコードも含めて)これらの情報をそれぞれ個別に持たせるのではなく、わざわざ1つに集約した専用ゾーンにしているのでしょうか。理由は、**ドメインをまたいだ検索を対称的に、かつシンプルに行えるようにするため**です。もし各ドメインが自分のゾーンの中だけにDC情報を抱える設計だった場合、あるドメインのクライアントが**別のドメイン**のDCを探そうとすると、まずその別ドメインのDNSゾーン名そのものを何らかの方法で事前に知っている必要があります(フォレスト内にどんなドメインが存在するかという情報がなければ、そもそも問い合わせ先のゾーン名を組み立てられません)。一方`_msdcs.<フォレストルートドメイン名>`という名前は、フォレストに参加しているどのドメインのメンバーからも、フォレストルートドメイン名さえわかれば必ず同じ1つの名前として導き出せます。この「誰から見ても同じ、単一の既知の場所」に全ドメイン分の情報を集約しておくことで、グローバルカタログの検索のようなフォレスト横断的な問い合わせも、特別な事前情報なしに成立するようになっているのです。
