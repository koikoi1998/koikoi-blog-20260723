---
title: "Windowsのプロセス・タスク・スレッドの違いを『上位1%』の視点で理解する——ゾンビプロセスとは何か"
description: "タスクマネージャで見るタスクとプロセスは同じものなのか。ゾンビプロセスとは何で、なぜ発生するのか。GoogleDriveがエクスプローラー上に表示されないとき、なぜタスクマネージャでプロセスを終了させると直るのかまでを体系的に理解する。"
...
- **この記事で得られること**: 「GoogleDriveがエクスプローラー上に表示されなくなり、タスクマネージャでプロセスを終了させたら直った」という実務でよくある対処法を出発点に、**プロセス・スレッド・タスクという似て非なる用語の違い**、**ゾンビプロセスとは何で、なぜ発生するのか**を体系的に理解します。
...
### ゾンビプロセスの居座りと、通常のフリーズの違い

上位1%のエンジニアは、「アプリケーションが固まった」という報告を受けたとき、まず**タスクマネージャの「詳細」タブでそのプロセスのCPU・メモリ使用率の推移を確認し、"何もしていない"のか"何かに詰まって処理し続けている"のかを切り分けます。** ゾンビプロセスに近い居座り状態は、多くの場合CPU使用率がほぼゼロのまま応答だけが返ってこない一方、無限ループのようなバグはCPU使用率が高止まりし続けます。この一手間が、同じ「反応がない」という症状でも、原因調査の方向性を大きく変えます。
