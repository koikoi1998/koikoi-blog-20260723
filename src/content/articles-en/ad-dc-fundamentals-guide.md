...
- **LDAP (Lightweight Directory Access Protocol)**: The standard protocol for searching, adding, modifying, and deleting entries in a directory service. AD DS accepts queries over LDAP. LDAP itself — its DNs, search filters, and how its ports are used differently — is covered in depth in [Understanding the LDAP Protocol](/en/articles/ad-ldap-protocol-guide).
...
| .NET Framework 4.8 Features (including WCF Services and TCP Port Sharing) | Feature | Not AD DS-specific — it's a **baseline feature enabled by default** that many of Windows Server's management tools and PowerShell modules rely on as their runtime. The relationship between .NET Framework and PowerShell itself is covered in depth in [Understanding .NET Framework and PowerShell](/en/articles/ad-dotnet-powershell-guide) |
