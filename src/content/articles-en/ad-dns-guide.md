...
- **Simplifying startup-time dependencies**: During a DC's boot sequence, the [Netlogon service](/en/articles/ad-netlogon-guide) dynamically registers a number of records about itself, including the SRV records mentioned earlier. If it's configured to look up its own real IP address as the DNS server for this, timing issues in that DC's own network stack or DNS service initialization can sometimes make this lookup unstable during startup. Pointing it at `127.0.0.1` (loopback) instead is a way to more reliably ensure that **"ask my own DNS service first" holds regardless of the overall network stack's initialization state.**
