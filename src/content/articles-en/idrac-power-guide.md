...
To achieve the A/B grid redundancy mentioned earlier, connecting all PSUs to a single PDU defeats the purpose. Only by physically separating the PDU for Grid A from the PDU for Grid B, and connecting each to a separate UPS and separate breaker, does redundancy of the power delivery path actually hold.
...
**3. Once the main rail is output, the motherboard's VRM further converts it into the finer-grained voltages each component needs (around 1V for the CPU, around 1.1V for memory, etc.).** Only once power supply has stabilized does POST (Power-On Self-Test) run, beginning the OS's boot process.
