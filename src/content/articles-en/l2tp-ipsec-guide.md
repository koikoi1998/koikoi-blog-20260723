...
The key point is that **ESP encrypts the UDP header (port 1701) along with everything else** (the detailed mechanics of ESP itself are explained later in "The Role IPsec Plays"). Because IPsec's transport mode leaves the original IP header intact and encrypts only its contents (in this case, UDP/L2TP/PPP/data — all of it), the only information visible from outside is "the source and destination global IP addresses" and "the fact that this is an ESP packet (IP protocol number 50)." The information that this is UDP port 1701 is itself hidden inside the encrypted payload and is invisible to devices along the path. This point matters again later, in "Common Misconceptions."
...
How certificates are issued in the first place, and what CSRs, CAs, and certificate chain validation each actually do — the PKI (Public Key Infrastructure) mechanism itself — is a general-purpose technology used far beyond IKE, in TLS (HTTPS), SSH, and many other contexts, and it's a substantial, independent topic. The separate article [Understanding PKI and Digital Certificates from a "Top 1%" Perspective — From Public-Key Cryptography to Certificate Chain Verification](/en/articles/pki-guide) dives deeply into it, from the roles of client certificates, server certificates, and certificate authorities through to the certificate chain verification procedure.
...
- **Misconception 4: "Any number of devices can connect simultaneously as long as they're behind the same NAT."**
