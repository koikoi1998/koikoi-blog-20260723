...
The key point is that **ESP encrypts the UDP header (port 1701) along with everything else** (the detailed mechanics of ESP itself are explained later in "The Role IPsec Plays"). The "original data (inner IP packet)" shown in the diagram is bundled together with UDP/L2TP/PPP and falls entirely inside what gets encrypted, so the only information visible from outside is "the source and destination global IP addresses" and "the fact that this is an ESP packet (IP protocol number 50)" — not just the UDP port 1701 information, but also the inner IP packet's own source (virtual IP) and destination (internal server IP) addresses, disappear into the encrypted payload and are invisible to devices along the path. It's worth being precise here: **this arrangement is called "transport mode" because ESP does not add a second, new outer IP header — not because the original inner IP packet stays in the clear.** (This exact misconception gets a fuller treatment later, in "Common Misconceptions," Misconception 4.)
...
<details>
<summary>Digging one level deeper into tunneling (why the illusion of a dedicated link holds up, plus two common questions)</summary>

**Why does merely "wrapping" the frame create the illusion of a dedicated link?** In its implementation, PPP treats "the lower layer that sends and receives frames" purely as an abstract interface — whether that's a telephone line's serial port or a tunnel over an IP network, PPP itself is designed not to care at all what medium lies underneath, as long as the send/receive contract holds: "hand over a frame and it reaches the other side; if a frame arrives from the other side, I can receive it." L2TP implements this lower-layer interface over an IP network in place of a physical serial line. Specifically, it takes a single frame handed off by PPP, wraps it unchanged as a UDP/IP payload, and sends it to a **single fixed destination corresponding to the already-established Tunnel ID and Session ID (discussed below)**. From PPP's point of view, the exact same contract as a dedicated link continues to hold, with only the implementation underneath swapped from a serial port to L2TP's send/receive processing.

With this groundwork in place, two common questions can now be answered clearly. **First: "Is the tunnel created in order to encrypt the PPP exchange?"** — the answer is no. L2TP itself has no encryption capability whatsoever (this is also covered later, in "Common Misconceptions"), and the tunnel's purpose is purely to emulate the one-to-one link that PPP requires on top of a shared IP network — it has nothing to do with encryption. Encryption is entirely the job of a separate layer, IPsec (ESP), covered later in "The Role IPsec Plays."

**Second: "IPsec already provides encryption, so why not start PPP negotiation directly instead of inserting the L2TP tunneling step?"** — what matters here is the difference between what IPsec (ESP) provides and what PPP needs. What IPsec (ESP) provides is "a confidential IP communication path between two IP addresses"; it does not, on its own, set up "the shape of a one-to-one link that PPP can run over" inside that path. Inserting L2TP provides, inside the already-encrypted path, that virtual link. In addition, L2TP's Tunnel ID/Session ID also serves to identify and multiplex individual PPP sessions when many users connect to a single VPN server simultaneously — a function that IPsec's SA (Security Association) alone cannot substitute for.

</details>

What L2TP actually uses to build this tunnel is **encapsulation in UDP/IP** (a choice already fixed in L2TP's own specification, RFC 2661, before it was ever combined with IPsec). Why ESP alone isn't enough to justify skipping UDP, and how the receiving side actually peels apart this multi-layered stack of headers, are answered in the next collapsible section.

<details>
<summary>Why did L2TP choose UDP, and how does the receiving side decapsulate it?</summary>

**ESP runs directly after the IP header as IP protocol number 50 and handles encryption and authentication on its own — it looks like that alone should be enough to get a packet to the VPN server, so why is a UDP header needed at all?** Encryption via ESP is a separate layer stacked on top afterward — there's no relationship of "UDP becomes unnecessary because ESP exists" to begin with. So why did L2TP itself choose UDP? There are mainly two reasons.

The first is that **it can be implemented entirely with ordinary user-space socket APIs.** In plain terms: handling an IP protocol other than TCP/UDP directly — like ESP (IP protocol number 50) — is like having to apply to the OS for a special, privileged counter before you're allowed to use it (on most operating systems, creating a raw socket demands root-equivalent privileges). UDP, by contrast, is like an ordinary counter anyone can walk up to — you just `bind()` to a specific port (1701 for L2TP), the kind of completely ordinary socket programming any application programmer can do. L2TP was originally meant to be implemented as ordinary software running on general-purpose servers rather than specialized hardware, which is why not requiring special privileges mattered so much in its design. In fact, `xl2tpd`, a commonly used L2TP implementation on Linux, runs as an ordinary, unprivileged daemon process that simply handles a UDP socket. Why a daemon's implementation can be self-contained as an ordinary user-space process is covered in [What Is a Daemon? Understanding Linux Background Processes from a "Top 1%" Perspective](/en/articles/linux-daemon-guide).

The second is **multiplexing by port number.** An intuitive way to picture this: think of an IP address as a building's street address, and an IP protocol number as the type of mailbox that building is allowed to have. A protocol-number-only scheme like ESP's is like a building that can only have one "ESP mailbox" — one slot, period. But a VPN server, as a single "building," needs several distinct services running at once: IKE (key exchange), L2TP (the tunnel), and NAT-T (NAT traversal). If L2TP had been designed to communicate purely by protocol number the way ESP does, it couldn't coexist with IKE and NAT-T at that same address. Inserting a transport layer like UDP gives that building "room numbers" — port numbers — so L2TP can live in room 1701, IKE in room 500, and NAT-T in room 4500, all listening independently at the same address at the same time. That's what "multiplexing by port number" means. For a deeper, more technical look at exactly how protocol numbers differ from port numbers, and why a port number can't simply be assigned to ESP after the fact, see [How TCP/UDP "Sessions" and Port Numbers Relate, from a "Top 1%" Perspective](/en/articles/tcp-udp-session-port-guide).

Note that, in actual L2TP/IPsec communication, this "L2TP uses UDP 1701" detail ends up encrypted, UDP header and all, by the ESP layered on top of it afterward, so the UDP port 1701 information itself becomes invisible to any third party along the path or to NAT devices (as covered earlier in "Misconception 3"). The reason L2TP uses UDP and the reason IPsec protects it are simply two entirely separate design decisions stacked on top of each other.

**So how does the receiving side actually peel this multi-layered stack of headers apart correctly?** The short answer: there is no rule in the OSI model itself that says a Layer 4 (transport-layer) header must always be inserted — the proof is ESP itself, which carries no Layer 4 header at all yet still travels and gets processed correctly. **What each layer's header is fundamentally doing is carrying an identifier that says "hand this content to which processing step next,"** and whether that identifier is the IP header's protocol number, the UDP header's port number, the L2TP header's Tunnel ID/Session ID, or the PPP header's Protocol field depends on which multiplexing problem that layer needs to solve. A protocol like ESP, which skips Layer 4 entirely, tells multiple peers apart using its own identifier, the **SPI (Security Parameters Index)**, in place of a port number.

With that lens, here's what the receiving side (the VPN server) actually does to peel the headers apart:

1. **Look at the outer IP header**: A **Protocol field** value of `50` identifies it as ESP, so the OS hands the packet to the IPsec processing code.
2. **Look at the ESP header**: The **SPI** identifies which of the VPN server's multiple concurrent IPsec SAs (the negotiated set of keys and cryptographic algorithms) to decrypt with. Decrypting reveals a UDP header inside.
3. **Look at the UDP header**: The destination port `1701` tells the OS to hand this payload to the process that handles L2TP (such as `xl2tpd`).
4. **Look at the L2TP header**: The combination of **Tunnel ID** and **Session ID** identifies exactly which client's, and which PPP session's, frame this is.
5. **Look at the PPP header**: The **Protocol field** value (`0x0021` for actual data, `0x8021` for an IPCP control message, and so on) determines whether this is actual data or a control message.
6. **What's left, finally, is the original inner IP packet**, which the PPP layer hands off to the OS's ordinary routing logic.

In other words: while the sending side stacks headers data → PPP → L2TP → UDP → ESP → outer IP, the receiving side works through it in **the reverse order, reading the identifier each header carried as it peels that header away.** As long as each layer carries an identifier pointing to the next destination, there's no absolute rule about how many layers there must be or what kind — it's simply the rational outcome of the multiplexing problem each layer needs to solve.

</details>

### L2TP's Message Types and Tunnel ID/Session ID
...
<details>
<summary>Why Is a PPP Header Still Needed After IPCP Completes?</summary>
