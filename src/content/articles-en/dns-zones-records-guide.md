...
<details>
<summary>Is the `_msdcs` zone only for finding the forest root domain's DC?</summary>

This is an easy point to misunderstand. Looking only at the zone name `_msdcs.<forest root domain name>`, it reads as if it were "a special zone for finding the forest root domain's DC." More precisely, though, **the "forest root domain name" part of that zone name is just describing where the zone itself lives (the root of its namespace).** Inside this one zone, you'll actually find **each domain's own "find that domain's DC" records, for every domain in the forest, not just the forest root — kept distinct by domain name** (for example, `_ldap._tcp.dc._msdcs.corp.example.com` exists inside this same `_msdcs` zone even when `corp.example.com` is a child domain). In other words, the design isn't "a separate `_msdcs` zone per domain" — it's "**one `_msdcs` zone for the whole forest, holding every domain's records together.**"

So why consolidate this into one dedicated zone, rather than having each domain hold this information (including the forest root's DC records) inside its own ordinary forward lookup zone? The reason is **to make cross-domain lookups symmetric and simple.** If each domain only held DC information inside its own zone, a client in one domain trying to find a DC in a *different* domain would first need to already know that other domain's DNS zone name by some other means (without knowing what domains exist in the forest, you can't even construct the query's target zone name in the first place). By contrast, the name `_msdcs.<forest root domain name>` can always be derived as the exact same single name by any member of any domain in the forest, as long as they know the forest root domain's name. Consolidating every domain's information into this "one place everyone already knows" is exactly what lets forest-wide lookups — like finding a global catalog server — work without needing any special prior knowledge.
