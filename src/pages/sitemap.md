...
<li data-subseries="handson"><a href="/articles/aws-vpc-endpoint-handson-guide">VPCエンドポイントでNATゲートウェイを経由せずS3にアクセスするハンズオン</a></li>
...
<li data-subseries="handson"><a href="/articles/ansible-jinja2-loops-handson-guide">Jinja2フィルターとloop・whenの落とし穴を体験するハンズオン</a></li>
...
<ol class="persona-route-list">
<li>STEP1〜STEP6の123記事(上のタブから確認できます)</li>
...
<p>全147記事を読み切り、シリーズ全体の設計思想を一貫して語れる状態を目指す、完全制覇ルートです。</p>
</div>
<ol class="persona-route-list">
<li>STEP1〜STEP7の146記事(上のタブから確認できます)</li>
<li><a href="/articles/voip-ss7-guide">VoIPとSS7、そして実際の通信経路を『上位1%』の視点で理解する</a></li>
</ol>
<div class="persona-bonus">🎉 <strong>これで全147記事読了です。</strong> シリーズ全体の構成は、次の「シリーズ一覧」でも振り返れます。</div>
</div>
</div>
</div>
...
EC2・VPCなど、AWSを使ううえで必ず直面する基礎的な疑問を深掘りするシリーズです。**読む順番の目安**: ① aws-ec2-networking-basics-guide → ② aws-ec2-webserver-handson-guide → ③ aws-iam-role-handson-guide → ④ aws-s3-static-website-handson-guide → ⑤ aws-vpc-handson-guide → ⑥ aws-rds-secrets-handson-guide → ⑦ aws-vpc-endpoint-handson-guide → ⑧ aws-ebs-snapshot-handson-guide。
...
- [VPCエンドポイントでNATゲートウェイを経由せずS3にアクセスするハンズオン](/articles/aws-vpc-endpoint-handson-guide) — NATゲートウェイの通過データ量課金を避けるハンズオン。ゲートウェイ型(無料)とインターフェイス型(有料)の違いまでを扱う(単体でも読めます)。
...
- [Jinja2フィルターとloop・whenの落とし穴を体験するハンズオン](/articles/ansible-jinja2-loops-handson-guide) — registerした変数の構造化されたデータ、whenがloopの各要素ごとに評価される仕様を扱うハンズオン。changed_whenによる変更検知のカスタマイズまでを扱う(単体でも読めます)。
